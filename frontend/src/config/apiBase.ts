...
const API_URLS = {
  staging:
    "https://qgp-staging.ashymushroom-85447e68.uksouth.azurecontainerapps.io",
...
export function detectEnvironment(): Environment {
  ...
  if (
    explicitEnv &&
    ["staging", "production", "development"].includes(explicitEnv)
  ) {
...
export async function validateEnvironmentMatch(): Promise<string | null> {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        "[API Config] Failed to fetch API version for environment validation",
      );
...
function logApiConfig(): void {
  ...
  if (envUrl) {
    if (env === "staging" && envUrl.includes("prod")) {
      console.error(
        "[API Config] WARNING: Staging frontend configured with production API URL!",
      );
    } else if (env === "production" && envUrl.includes("staging")) {
      console.error(
        "[API Config] WARNING: Production frontend configured with staging API URL!",
      );
    }
  }
}
