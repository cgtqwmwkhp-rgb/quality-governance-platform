...
import {
  auditsApi,
  AuditTemplateDetail,
  AuditSection as ApiSection,
  AuditQuestion as ApiQuestion,
  AuditTemplateUpdate,
} from "../api/client";
...
function OptionsEditor({
  ...
  const [opts, setOpts] = useState(
    initial.length > 0
      ? initial
      : [{ value: "option_1", label: "Option 1", score: 0 }],
  );
  ...
  const addOption = () => {
    ...
    setOpts([
      ...opts,
      { value: `option_${idx}`, label: `Option ${idx}`, score: 0 },
    ]);
  };
          ...
          {opts.map((opt, idx) => (
            <div key={idx} className="flex items-center gap-2">
              ...
              <Input
                ...
                value={opt.score ?? 0}
                onChange={(e) =>
                  updateOption(idx, "score", parseFloat(e.target.value) || 0)
                }
              ...
              <label
                ...
                title="Triggers audit finding"
              >
                <input
                  type="checkbox"
                  checked={opt.triggers_finding || false}
                  onChange={(e) =>
                    updateOption(idx, "triggers_finding", e.target.checked)
                  }
                  className="w-3 h-3"
                />
                Finding
    ...
    async (newType: string) => {
      ...
      try {
        ...
        if (newType === "yes_no" || newType === "pass_fail") {
          ...
          update["options"] = [
            {
              value: newType === "yes_no" ? "yes" : "pass",
              label: newType === "yes_no" ? "Yes" : "Pass",
              score: 1,
              triggers_finding: false,
            },
            {
              value: newType === "yes_no" ? "no" : "fail",
              label: newType === "yes_no" ? "No" : "Fail",
              score: 0,
        ...
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 space-y-3">
              ...
              <div>
                <label htmlFor={`q-desc-${question.id}`} className="sr-only">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div>
                  <label htmlFor={`q-type-${question.id}`} className="sr-only">
                    Question type
                  </label>
                  <Select
                ...
                <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer min-h-[44px] px-2">
                  <input
                    ...
                    onChange={(e) =>
                      handleToggle("is_required", e.target.checked)
                    }
                  ...
                  <CardContent className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          debouncedSave("help_text", e.target.value)
                        }
                    ...
                    <div>
                      <label
                        htmlFor={`q-maxscore-${question.id}`}
                        className="block text-sm font-medium text-foreground mb-1"
                      >
                        Max Score
                      </label>
      ...
      <Card>
        <CardHeader className="pb-0">
          <div className="flex items-center gap-3">
            <div
              className="p-2 rounded-lg bg-surface text-muted-foreground"
              aria-hidden="true"
            >
            ...
            <div className="flex-1 space-y-1">
              <label htmlFor={`sec-title-${section.id}`} className="sr-only">
                Section title
              </label>
              <Input
              ...
              />
              <label htmlFor={`sec-desc-${section.id}`} className="sr-only">
                Section description
              </label>
              <Input
      ...
      <Dialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete &quot;{section.title}&quot; and
              all {activeQuestions.length} questions in it? This cannot be
              undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
...
function SettingsPanel({ template, onSave }: SettingsPanelProps) {
  ...
  const [scoringMethod, setScoringMethod] = useState(
    template.scoring_method || "percentage",
  );
  const [passingScore, setPassingScore] = useState(
    template.passing_score ?? 80,
  );
  const [auditType, setAuditType] = useState(
    template.audit_type || "inspection",
  );
  const [saving, setSaving] = useState(false);
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {CATEGORIES.map((cat) => (
                ...
                >
                  <cat.icon
                    className={`w-5 h-5 ${category === cat.id ? "text-primary" : "text-muted-foreground"}`}
                  />
                ...
                {(section.questions || [])
                  ...
                  .map((question, qIndex) => {
                    const qType = QUESTION_TYPES.find(
                      (t) => t.type === question.question_type,
                    );
                    return (
                      <div
                        key={question.id}
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2 mt-2 flex-wrap">
                            ...
                            {question.risk_category && (
                              <Badge
                                variant={
                                  question.risk_category === "critical"
                                    ? "critical"
                                    : question.risk_category === "high"
                                      ? "high"
                                      : question.risk_category === "medium"
                                        ? "medium"
                                        : "low"
                                }
                              >
...
export default function AuditTemplateBuilder() {
  ...
  useEffect(() => {
    const load = async () => {
      if (!isEditing) {
        try {
          ...
          window.history.replaceState(
            null,
            "",
            `/audit-templates/${response.data.id}/edit`,
          );
        } catch (err) {
      ...
      try {
        ...
        setError(
          "Failed to load template. It may not exist or you may not have access.",
        );
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const handleSaveSettings = useCallback(
    async (data: AuditTemplateUpdate) => {
      if (!template) return;
      try {
    ...
    try {
      ...
      const message =
        (err as { response?: { data?: { detail?: string } } })?.response?.data
          ?.detail || "Failed to publish. Ensure at least one question exists.";
      ...
      nameDebounceRef.current = setTimeout(async () => {
        if (template) {
          try {
            ...
            /* will be saved on explicit save */
          }
        }
      }, 1000);
    },
  ...
  const stats = useMemo(() => {
    ...
    const allQuestions = activeSections.flatMap((s) =>
      (s.questions || []).filter((q) => q.is_active),
    );
    return {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {!template.is_published && (
            <Button
              ...
              disabled={saving || stats.questions === 0}
            >
              {saving ? (
                <Loader2 className="w-4 h-4 animate-spin" />
              ) : (
                ...
                <CardContent className="space-y-3">
                  {[
                    ...
                    { label: "Required", value: stats.required },
                    {
                      ...
                      value: `${template.passing_score ?? 0}%`,
                    },
                  ].map((stat) => (
                    <div
                      key={stat.label}
      ...
      >
        {activeTab === "preview" && <PreviewPanel template={template} />}
      </div>
          ...
          onApply={async (generatedSections) => {
            ...
            try {
              for (let i = 0; i < generatedSections.length; i++) {
                ...
                try {
                  const sectionRes = await auditsApi.createSection(
                    template.id,
                    {
                      title: gs.title,
                  ...
                  for (let qi = 0; qi < gs.questions.length; qi++) {
                    ...
                    const sanitizedType = VALID_QUESTION_TYPES.has(q.type)
                      ? q.type
                      : "yes_no";
              ...
              if (failures === 0) {
                showToast(
                  `${successes} AI-generated sections added`,
                  "success",
                );
              } else if (successes > 0) {
                showToast(
                  `${successes} sections added, ${failures} failed`,
                  "error",
                );
              } else {
