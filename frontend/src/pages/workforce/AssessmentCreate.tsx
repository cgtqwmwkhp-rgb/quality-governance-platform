...
export default function AssessmentCreate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      const res = await workforceApi.createAssessment({
      ...
      navigate(`/workforce/assessments/${res.data.id}/execute`)
    } catch (err) {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            {t('workforce.assessments.create_subtitle')}
          </p>
        </div>
      </div>
      ...
      <Card className="bg-card border-border">
        <CardHeader>
          <h2 className="text-lg font-semibold text-foreground">
            {t('workforce.assessments.details')}
          </h2>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label
                htmlFor="assessmentcreate-field-2"
                className="block text-sm font-medium text-foreground mb-1"
              >
                {t('workforce.common.asset_type')}
              </label>
              <select
                id="assessmentcreate-field-2"
                value={assetTypeId}
            ...
            <div>
              <label
                htmlFor="assessmentcreate-field-3"
                className="block text-sm font-medium text-foreground mb-1"
              >
                {t('common.title')}
              </label>
              <input
                id="assessmentcreate-field-3"
                type="text"
                value={title}
            ...
            <div>
              <label
                htmlFor="assessmentcreate-field-4"
                className="block text-sm font-medium text-foreground mb-1"
              >
                {t('common.location')}
              </label>
              <input
                id="assessmentcreate-field-4"
                type="text"
                value={location}
            ...
            <div>
              <label
                htmlFor="assessmentcreate-field-5"
                className="block text-sm font-medium text-foreground mb-1"
              >
                {t('workforce.common.scheduled_date')}
              </label>
              <input
                id="assessmentcreate-field-5"
                type="date"
                value={scheduledDate}
            ...
            <div>
              <label
                htmlFor="assessmentcreate-field-6"
                className="block text-sm font-medium text-foreground mb-1"
              >
                {t('common.notes')}
              </label>
              <textarea
                id="assessmentcreate-field-6"
                value={notes}
