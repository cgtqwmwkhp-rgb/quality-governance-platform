...
/** Soft-gate warning surfaced from AssessmentRun `/start` response fields. */
...
export function isCompetencyGateBlockedError(error: unknown): boolean {
  ...
  const envelope = data as {
    error?: { code?: string; details?: { reason?: string; mode?: string } }
...
export function readCompetencyGateWarning(
  run: Pick<
    AssessmentRun,
    'competency_gate_cleared' | 'competency_gate_reason' | 'competency_gate_mode' | 'competency_gate_blocked' | 'competency_gate_message'
  >,
    ...
    try {
      ...
      if (run.status === 'draft' && !autoStartedRunIds.current.has(id)) {
        try {
          ...
          // Hard gate: keep draft loaded and show remediation (do not retry-loop).
          autoStartedRunIds.current.add(id)
      ...
      try {
        const tmpl = await auditsApi.getTemplate(run.template_id)
        template = tmpl.data
      } catch {
        ...
        <Card className="border-destructive" data-testid="competency-gate-blocked">
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              Mode: hard · {assessment.reference_number} · {engineerName}
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="secondary" onClick={() => navigate('/workforce/assessments')}>
                Back to assessments
              </Button>
            </div>
          </CardContent>
      ...
      <div className="space-y-6 pb-8">
        ...
        <Card className="bg-card border-border">
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <div className="rounded-lg border border-dashed border-border bg-muted/20 min-h-[80px] flex items-center justify-center p-4">
                <input
                  ...
                  placeholder={t('workforce.assessments.signature_placeholder')}
            ...
            >
              {submitting ? 'Submitting...' : t('workforce.common.submit_assessment')}
            </Button>
          </CardContent>
        </Card>
      </div>
      ...
      {gateWarning && (
        ...
        >
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            {assessment?.engineer_id != null && (
              <Button
                variant="outline"
                size="sm"
                onClick={() => navigate(`/workforce/engineers/${assessment.engineer_id}`)}
              >
                View tickets / passport
              </Button>
            )}
