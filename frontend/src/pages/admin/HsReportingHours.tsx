...
export default function HsReportingHours() {
  ...
  const updateRow = (year: number, patch: Partial<DraftRow>) => {
    setRows((current) => current.map((row) => (row.reporting_year === year ? { ...row, ...patch } : row)))
  }
      ...
      <Card>
        ...
        <CardContent>
                    ...
                    <tr key={row.reporting_year} className="border-b align-top">
                      ...
                      <td className="py-3 pr-3">
                        ...
                        <Input
                          ...
                          onChange={(e) => updateRow(row.reporting_year, { manual_hours: e.target.value })}
                          className="w-[10rem]"
                        />
                      </td>
          ...
          <p className="text-xs text-muted-foreground mt-4">
            ...
            are the denominator for LTIFR and AFR (per 100,000 hours).
          </p>
        </CardContent>
      </Card>
    </div>
