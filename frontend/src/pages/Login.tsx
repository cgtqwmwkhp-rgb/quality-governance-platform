...
export default function Login({ onLogin }: LoginProps) {
  ...
  const slowWarningTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const handleOAuthCallback = useCallback(async (): Promise<boolean> => {
    ...
    if (hash && hash.includes("id_token")) {
      ...
      if (idToken) {
        ...
        if (accessToken) {
          const durationMs =
            Date.now() - (requestStartRef.current || Date.now());
  ...
  // Derived state
  const isLoading = ["submitting", "spinner_visible", "slow_warning"].includes(
    loginState,
  );
      ...
      <div className="w-full max-w-md relative animate-fade-in">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">
            Sign in to manage your governance
          </p>
        </div>
