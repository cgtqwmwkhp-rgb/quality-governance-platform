...
export default function ComplianceAutomation() {
  ...
  const [riddorSubmissions, setRiddorSubmissions] = useState<
    RiddorSubmission[]
  >([]);
  const [expiringSummary, setExpiringSummary] =
    useState<ExpiringSummary | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (updatesRes.status === "fulfilled") {
        setUpdates(
          ((updatesRes.value.data as Record<string, unknown>)[
            "updates"
          ] as typeof updates) ?? [],
        );
      }
      if (certsRes.status === "fulfilled") {
        setCertificates(
          ((certsRes.value.data as Record<string, unknown>)[
            "certificates"
      ...
      if (auditsRes.status === "fulfilled") {
        setAudits(
          ((auditsRes.value.data as Record<string, unknown>)[
            "audits"
          ] as typeof audits) ?? [],
        );
      }
      ...
      if (gapRes.status === "fulfilled") {
        setGapAnalyses(
          ((gapRes.value.data as Record<string, unknown>)[
            "analyses"
          ] as typeof gapAnalyses) ?? [],
        );
      }
      if (riddorRes.status === "fulfilled") {
        setRiddorSubmissions(
          ((riddorRes.value.data as Record<string, unknown>)[
            "submissions"
          ] as typeof riddorSubmissions) ?? [],
        );
      }
  ...
  const handleMarkReviewed = async (updateId: number) => {
    ...
    try {
      ...
      setUpdates((prev) =>
        prev.map((u) => (u.id === updateId ? { ...u, is_reviewed: true } : u)),
      );
  ...
  const handleRunGapAnalysis = async (regulatoryUpdateId?: number) => {
    ...
    try {
      ...
      setGapAnalyses(
        ((gapRes.data as Record<string, unknown>)[
          "analyses"
        ] as typeof gapAnalyses) ?? [],
      );
    } catch (err) {
  ...
  const handleScheduleAudit = async () => {
    ...
    try {
      await complianceAutomationApi.scheduleAudit({
        ...
        standard_ids: auditForm.standard_ids
          ? auditForm.standard_ids.split(",").map((s) => s.trim())
          : undefined,
      });
      ...
      setAudits(
        ((res.data as Record<string, unknown>)["audits"] as typeof audits) ??
          [],
      );
    } catch (err) {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-primary to-primary-hover rounded-xl p-5 text-primary-foreground relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="text-4xl font-bold mb-0.5">
              {complianceScore.overall_score.toFixed(1)}%
            </div>
        ...
        >
          ...
          <div className="text-2xl font-bold text-foreground">
            {expiringSummary
              ? expiringSummary.expiring_90_days
              : certificates.filter((c) => c.status === "expiring_soon").length}
          </div>
          <div className="text-sm text-muted-foreground">
            {expiringSummary
              ? `${expiringSummary.expired} expired · ${expiringSummary.total_critical} critical`
              : "Within 90 days"}
          </div>
        </button>
          ...
          {updates.length === 0 && (
            <div className="bg-card border border-border rounded-xl p-12 text-center">
              ...
              <p className="text-muted-foreground text-sm mt-1">
                No regulatory updates require your attention.
              </p>
            </div>
          )}
          {updates.map((update) => (
              ...
              <div className="space-y-3">
                {gapAnalyses.map((ga) => (
                  ...
                  >
                    <button
                      onClick={() =>
                        setExpandedGapId(expandedGapId === ga.id ? null : ga.id)
                      }
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {formatDate(ga.created_at)} · {ga.total_gaps} gap
                            {ga.total_gaps !== 1 ? "s" : ""}
                            {ga.critical_gaps > 0 &&
                              ` · ${ga.critical_gaps} critical`}
                    ...
                    {expandedGapId === ga.id && ga.gaps && (
                      <div className="border-t border-border p-4 space-y-3 bg-muted/20">
                        {(Array.isArray(ga.gaps) ? ga.gaps : []).map(
                          (gap, idx) => (
                            <div
                              key={idx}
        ...
        <div className="space-y-4">
          ...
          {expiringSummary &&
            (expiringSummary.expired > 0 ||
              expiringSummary.expiring_7_days > 0 ||
              expiringSummary.total_critical > 0) && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-3 text-center">
                  <div className="text-xl font-bold text-destructive">
                    {expiringSummary.expired}
                  </div>
                  <div className="text-xs text-destructive/80">Expired</div>
                </div>
                <div className="bg-orange-500/10 border border-orange-500/20 rounded-lg p-3 text-center">
                  <div className="text-xl font-bold text-orange-500">
                    {expiringSummary.expiring_7_days}
                  </div>
                ...
                <div className="bg-warning/10 border border-warning/20 rounded-lg p-3 text-center">
                  <div className="text-xl font-bold text-warning">
                    {expiringSummary.expiring_30_days}
                  </div>
                  <div className="text-xs text-warning/80">Within 30 days</div>
                </div>
                <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-3 text-center">
                  <div className="text-xl font-bold text-destructive">
                    {expiringSummary.total_critical}
                  </div>
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            ...
            {certificates.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-muted-foreground text-sm mt-1">
                  Add certificates to track their expiry dates.
                </p>
              </div>
            ) : (
                    ...
                    >
                      <div className="p-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3 shrink-0 ml-4">
                          ...
                          <button
                            onClick={() =>
                              setCertDetailId(isExpanded ? null : cert.id)
                            }
                            className={cn(
                    ...
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <div className="text-right">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {formatDate(audit.next_due_date)}
                          </p>
                        </div>
                        <button
                          onClick={() =>
                            showToast(
                              `Audit "${audit.name}" would launch in the Audit Builder`,
                              "info",
                            )
                          }
          ...
          <div className="bg-card border border-border rounded-xl p-6">
            ...
            <div className="space-y-5">
              {Object.entries(complianceScore.breakdown).length > 0 ? (
                Object.entries(complianceScore.breakdown).map(
                  ([name, data]) => {
                    const barColor =
                      data.score >= 90
                        ? "bg-success"
                    ...
                    return (
                      <div key={name}>
                        ...
                        <p className="text-xs text-muted-foreground mt-1.5">
                          {data.clauses_compliant}/{data.clauses_total} clauses
                          compliant · {data.gaps} gap
                          {data.gaps !== 1 ? "s" : ""} identified
                        </p>
                      </div>
                    );
                  },
          ...
          <div className="space-y-6">
            <div className="bg-card border border-border rounded-xl p-6">
              ...
              <div className="space-y-2.5">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-foreground text-sm font-medium">
                          {data.gaps} gap{data.gaps !== 1 ? "s" : ""} identified
                        </p>
                        <p className="text-muted-foreground text-xs">
                          {standard} ·{" "}
                          {data.clauses_total - data.clauses_compliant} clauses
                          non-compliant
                        </p>
                      </div>
                ...
                {Object.entries(complianceScore.breakdown).filter(
                  ([, d]) => d.gaps > 0,
                ).length === 0 && (
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground text-xs">
                      Assess compliance across all overdue audits
                    </p>
                  </div>
                </button>
              </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-muted-foreground text-sm mb-4">
                  Automatically detect reportable incidents and prepare RIDDOR
                  submissions to the Health & Safety Executive.
                </p>
                <a
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            ...
            {riddorSubmissions.length === 0 ? (
              <div className="p-12 text-center">
                <CheckCircle className="w-12 h-12 text-success mx-auto mb-4" />
                <p className="text-foreground font-medium">
                  No RIDDOR submissions
                </p>
                <p className="text-muted-foreground text-sm mt-1">
                  Reportable incidents will appear here automatically when
                  detected.
                </p>
              </div>
            ) : (
                            ...
                            <span
                              className={cn(
                                ...
                                sub.submission_status === "submitted"
                                  ...
                                  : sub.submission_status === "ready_to_submit"
      ...
      {showCertModal && (
        ...
        >
          <div
            className="bg-card border border-border rounded-xl w-full max-w-lg shadow-lg animate-scale-in"
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCertForm((f) => ({ ...f, entity_id: e.target.value }))
                    }
                    className="w-full px-3 py-2 bg-background border border-border rounded-lg text-foreground text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-all"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCertForm((f) => ({ ...f, issuing_body: e.target.value }))
                  }
                  className="w-full px-3 py-2 bg-background border border-border rounded-lg text-foreground text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-all"
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAuditForm((f) => ({ ...f, description: e.target.value }))
                  }
                  className="w-full px-3 py-2 bg-background border border-border rounded-lg text-foreground text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-all"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    ...
                    <option value="environmental_audit">
                      Environmental Audit
                    </option>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setAuditForm((f) => ({ ...f, frequency: e.target.value }))
                    }
                    className="w-full px-3 py-2 bg-background border border-border rounded-lg text-foreground text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-all"
                  >
