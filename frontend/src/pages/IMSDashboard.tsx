...
export default function IMSDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "mapping" | "audit" | "review" | "isms"
  >("overview");
  const [selectedStandard, setSelectedStandard] = useState<string | null>(null);
  const [dashboardData, setDashboardData] =
    useState<IMSDashboardResponse | null>(null);
  ...
  const standards: StandardDisplay[] = (dashboardData?.standards ?? []).map(
    (s) => {
          ...
          <div className="text-right">
            ...
            {complianceCoverage && (
              <div className="flex items-center gap-1 text-primary-foreground/80 mt-1">
                ...
                <span>
                  {complianceCoverage.coverage_percentage}% evidence coverage
                </span>
              </div>
            )}
          </div>
              ...
              >
                ...
                {standard.setupRequired && (
                  <p className="text-xs text-muted-foreground mt-2">
                    No controls configured yet. Add controls to track
                    compliance.
                  </p>
                )}
              </div>
          ...
          {uvdb && (
            <div className="bg-card rounded-xl p-6 border border-border hover:border-border-strong transition-colors">
              ...
              {uvdb.status !== "not_started" && (
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">
                    {uvdb.completed_audits} completed
                  </span>
                  <span className="text-muted-foreground">
                    {uvdb.active_audits} active
                  </span>
                </div>
              )}
            </div>
              ...
              {planetMark.status !== "not_configured" && (
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground capitalize">
                    {planetMark.certification_status}
                  </span>
                  {planetMark.reduction_vs_previous !== null && (
                    <span
                      className={
                        planetMark.reduction_vs_previous > 0
                          ? "text-success"
                          : "text-destructive"
                      }
                    >
                      {planetMark.reduction_vs_previous > 0 ? "-" : "+"}
                      {Math.abs(planetMark.reduction_vs_previous)}%
                    </span>
                  )}
                </div>
              )}
            ...
            <div className="space-y-4">
              {[
                ...
                },
                {
                  ...
                  unit: "%",
                  status:
                    (complianceCoverage?.coverage_percentage ?? 0) >= 80
                      ? ("good" as const)
                      : ("warning" as const),
                },
                {
              ...
              {[
                {
                  ...
                  detail:
                    standards.length > 0
                      ? `${standards.length} standards tracked`
                ...
                },
                {
                  ...
                  detail: isms
                    ? `${isms.controls.implemented}/${isms.controls.applicable} controls implemented`
                    : (dashboardData?.isms_error ?? "Not configured"),
                  icon: Lock,
                ...
                },
                {
                  ...
                  detail:
                    uvdb && uvdb.status !== "not_started"
                      ? `${uvdb.completed_audits} audits completed`
                      : "No audits yet",
                  icon: Award,
                ...
                },
                {
                  ...
                  detail:
                    planetMark && planetMark.status !== "not_configured"
                      ? `${planetMark.total_emissions?.toFixed(1)} tCO2e`
                      : "Not configured",
                  icon: Leaf,
                  color: "text-teal-500",
                },
                {
                  ...
                  detail: complianceCoverage
                    ? `${complianceCoverage.total_evidence_links} evidence links`
                    : "Not configured",
                  icon: CheckCircle2,
                  color: "text-success",
                },
        ...
        <div className="bg-card rounded-xl border border-border overflow-hidden">
          <div className="p-4 bg-muted border-b border-border">
            <h3 className="font-bold text-foreground">
              Annex SL Cross-Standard Mapping
            </h3>
            <p className="text-sm text-muted-foreground">
              Common requirements across ISO management system standards
            </p>
          </div>
          {standards.length >= 2 ? (
                  ...
                  {[
                    {
                      ...
                      desc: "Understanding the organization and its context",
                    },
                    {
                      clause: "5.1 Leadership & Commitment",
                      desc: "Top management commitment",
                    },
                    {
                      clause: "6.1 Risks & Opportunities",
                      desc: "Actions to address risks and opportunities",
                    },
                    {
                      clause: "7.2 Competence",
                      desc: "Determining necessary competence",
                    },
                    {
                      clause: "9.2 Internal Audit",
                      desc: "Planning and conducting internal audits",
                    },
                    {
                      clause: "10.2 Nonconformity & Corrective Action",
                      desc: "Reacting to nonconformities",
                    },
                  ].map((mapping, i) => (
            ...
            <div className="p-8 text-center">
              <p className="text-muted-foreground">
                Add at least 2 standards to view cross-standard mapping.
              </p>
            </div>
          )}
        </div>
      ...
      {activeTab === "audit" && (
        <div className="bg-card rounded-xl border border-border">
          <div className="p-4 bg-muted border-b border-border flex items-center justify-between">
            ...
            <Button onClick={() => navigate("/audits")}>Plan New Audit</Button>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-card rounded-xl border border-border">
            <div className="p-4 bg-muted border-b border-border">
              ...
              <p className="text-sm text-muted-foreground">
                Data readiness across all modules
              </p>
            </div>
            <div className="p-4 space-y-3">
              {[
                ...
                },
                {
                  ...
                  source: "Planet Mark",
                  trend:
                    planetMark?.reduction_vs_previous &&
                    planetMark.reduction_vs_previous > 0
                      ? "improving"
                      : "stable",
                },
                {
                  ...
                  status:
                    complianceCoverage &&
                    complianceCoverage.total_evidence_links > 0
                      ? "Complete"
                      : "Pending",
                  ...
                  <div className="relative w-40 h-40 mx-auto mb-6">
                    <svg className="w-full h-full transform -rotate-90">
                      ...
                      <circle
                        ...
                        strokeWidth="10"
                        fill="transparent"
                        strokeDasharray={`${(completed / total) * 440} 440`}
        ...
        <div className="space-y-6">
          {!isms ? (
            <div className="bg-card rounded-xl border border-border p-8 text-center">
              ...
              <p className="text-foreground font-medium">
                ISO 27001 ISMS Not Configured
              </p>
              <p className="text-muted-foreground text-sm mt-1">
                {dashboardData?.isms_error ||
                  "The ISMS module needs to be set up to display data here."}
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <div className="bg-card rounded-xl p-5 border border-border">
                  ...
                  <div className="text-3xl font-bold text-foreground">
                    {isms.controls.implemented}/{isms.controls.applicable}
                  </div>
                  <div className="text-sm text-success mt-1">
                    {isms.controls.implementation_percentage}% Implemented
                  </div>
                </div>
                ...
                <div className="bg-card rounded-xl p-5 border border-border">
                  ...
                  <div className="text-sm text-destructive mt-1">
                    {isms.risks.high_critical} High/Critical
                  </div>
                </div>
                ...
                <div className="bg-card rounded-xl p-5 border border-border">
                  ...
                  <div className="text-sm text-muted-foreground mt-1">
                    {isms.incidents.last_30_days} in last 30 days
                  </div>
                </div>
              ...
              {isms.domains.length > 0 && (
                <div className="bg-card rounded-xl border border-border">
                  <div className="p-4 bg-muted border-b border-border">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {isms.controls.total} controls across{" "}
                      {isms.domains.length} themes
                    </p>
                  </div>
                  ...
                  <div className="p-6">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                      ...
                      <div className="text-center p-4 bg-muted rounded-lg border border-border">
                        <div className="text-3xl font-bold text-muted-foreground">
                          {isms.controls.total - isms.controls.applicable}
                        </div>
                        <div className="text-sm text-muted-foreground">
                          Excluded
                        </div>
                    ...
                    <div className="text-center">
                      <p className="text-muted-foreground text-sm">
                        The Statement of Applicability documents all{" "}
                        {isms.controls.total} Annex A controls, their
                        applicability status, implementation status, and
