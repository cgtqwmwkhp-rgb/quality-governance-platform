...
export const QUESTION_TYPES: {
  ...
  { type: 'yes_no', label: 'Yes / No', icon: ToggleLeft, description: 'Binary choice' },
  {
    ...
    label: 'Yes / No / N/A',
    icon: ToggleRight,
    description: 'With not applicable option',
  },
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <label
              htmlFor={`auto-action-${question.id}`}
              className="flex items-center gap-2 text-sm text-foreground cursor-pointer"
            >
              <input
                id={`auto-action-${question.id}`}
                type="checkbox"
              ...
              {(question.options || []).map((option, idx) => (
                <div key={option.id} className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleOptionUpdate(option.id, { score: parseInt(e.target.value) || 0 })
                    }
                    placeholder="Score"
                    className="w-16 px-2 py-1 bg-input border border-input rounded text-sm text-foreground text-center"
                  />
          ...
          {showAdvanced && (
            <div className="grid grid-cols-2 gap-3 p-3 bg-muted rounded-lg">
              <div>
                <label
                  htmlFor={`iso-clause-${question.id}`}
                  className="block text-xs text-muted-foreground mb-1"
                >
                  ISO Clause
                </label>
                <input
                  id={`iso-clause-${question.id}`}
                  type="text"
              ...
              <div>
                <label
                  htmlFor={`risk-level-${question.id}`}
                  className="block text-xs text-muted-foreground mb-1"
                >
                  Risk Level
                </label>
                <select
                  id={`risk-level-${question.id}`}
                  value={question.riskLevel || ''}
                  onChange={(e) =>
                    onUpdate(question.id, { riskLevel: e.target.value as Question['riskLevel'] })
                  }
                  className="w-full px-2 py-1 bg-input border border-input rounded text-sm text-foreground"
                >
