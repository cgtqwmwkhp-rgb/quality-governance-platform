...
export default function PlanetMark() {
  ...
  const [setupRequired, setSetupRequired] =
    useState<SetupRequiredResponse | null>(null);
  ...
  const handleAddEmission = async () => {
    if (!emissionForm.source_name.trim() || !emissionForm.category.trim()) {
      showToast("Source name and category are required", "error");
      return;
    }
    if (!currentYear) {
      showToast("No reporting year selected", "error");
      return;
    }
    try {
  ...
  const handleAddAction = async () => {
    if (!actionForm.action_name.trim() || !actionForm.description.trim()) {
      showToast("Action name and description are required", "error");
      return;
    }
    if (!currentYear) {
      showToast("No reporting year selected", "error");
    ...
    try {
      ...
      await planetMarkApi.createAction(currentYear.id, {
        ...
        time_bound: actionForm.deadline
          ? new Date(actionForm.deadline).toISOString()
          : new Date().toISOString(),
        expected_reduction_pct: actionForm.target_reduction_percent
          ? parseFloat(actionForm.target_reduction_percent)
          : undefined,
      });
  ...
  const transformAction = (
    ...
    progress_percent:
      apiAction.status === "completed"
        ? 100
        : apiAction.status === "in_progress"
          ? 50
          : 0,
    is_overdue: apiAction.due_date
      ? new Date(apiAction.due_date) < new Date() &&
        apiAction.status !== "completed"
      : false,
  });
    ...
    async (isRetry = false) => {
      ...
      try {
        ...
        if (transformedYears.length > 0) {
          try {
            const actionsResponse = await planetMarkApi.listActions(
              transformedYears[0]!.id,
            );
            const transformedActions =
              actionsResponse.data.map(transformAction);
            // Sort by deadline for deterministic ordering
            transformedActions.sort((a, b) =>
          ...
          try {
            const scope3Response = await planetMarkApi.getScope3(
              transformedYears[0]!.id,
            );
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                Net-Zero Journey • GHG Protocol Aligned
              </p>
            </div>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-background text-foreground p-6">
      ...
      {showEmissionModal && (
        <div
          ...
          aria-modal="true"
          onClick={(e) => {
            if (e.target === e.currentTarget) setShowEmissionModal(false);
          }}
          onKeyDown={(e) => {
            if (e.key === "Escape") setShowEmissionModal(false);
          }}
        >
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-lg">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={() => setShowEmissionModal(false)}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={emissionForm.source_name}
                  onChange={(e) =>
                    setEmissionForm((f) => ({
                      ...f,
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEmissionForm((f) => ({ ...f, scope: e.target.value }))
                    }
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="number"
                    value={emissionForm.activity_data}
                    onChange={(e) =>
                      setEmissionForm((f) => ({
                        ...f,
                        activity_data: e.target.value,
                      }))
                    }
                ...
                <div>
                  ...
                  <select
                    value={emissionForm.activity_unit}
                    onChange={(e) =>
                      setEmissionForm((f) => ({
                        ...f,
                        activity_unit: e.target.value,
                      }))
                    }
                    className="w-full px-3 py-2 bg-background border border-border rounded-lg text-foreground focus:ring-2 focus:ring-primary/50 focus:border-primary"
                  >
              ...
              <div>
                ...
                <textarea
                  value={emissionForm.notes}
                  onChange={(e) =>
                    setEmissionForm((f) => ({ ...f, notes: e.target.value }))
                  }
              ...
              <button
                ...
                disabled={
                  emissionSubmitting ||
                  !emissionForm.source_name.trim() ||
                  !emissionForm.category.trim()
                }
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="number"
                    value={actionForm.target_reduction_percent}
                    onChange={(e) =>
                      setActionForm((f) => ({
                        ...f,
                        target_reduction_percent: e.target.value,
                      }))
                    }
                    placeholder="e.g. 5"
                    min="0"
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                <select
                  ...
                  onChange={(e) =>
                    setCurrentYear(
                      years.find((y) => y.id === parseInt(e.target.value)) ??
                        null,
                    )
                  }
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="text-center">
                <div className="text-3xl font-bold text-primary-foreground">
                  {currentYear.total_emissions.toFixed(1)}
                </div>
              ...
              <div className="text-center">
                <div className="text-3xl font-bold text-primary-foreground">
                  {currentYear.emissions_per_fte.toFixed(2)}
                </div>
                <div className="text-primary-foreground/80 text-sm">
                  tCO₂e/FTE
                </div>
      ...
      {loadState === "success" && years.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 bg-card rounded-xl border border-border">
          ...
          <p className="text-muted-foreground mb-4">
            Start tracking your carbon emissions by adding your first reporting
            year.
          </p>
            ...
            <div className="space-y-6">
                ...
                {[
                  {
                    ...
                    detail: "Fleet diesel, Natural gas",
                  },
                  {
                    scope: "Scope 2",
                  ...
                  },
                  {
                    ...
                    icon: Globe,
                    label: "Value Chain",
                    detail: "Travel, Waste, Suppliers",
                  },
                ].map((scope) => {
                  const Icon = scope.icon;
                  const pct = (
                    (scope.value / currentYear.total_emissions) *
                    100
                  ).toFixed(1);
                  return (
                    <div
                  ...
                  <div className="p-4 bg-slate-700 border-b border-slate-600 flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-sm text-gray-400">
                        {completedActions}/{actions.length} actions completed
                      </p>
                    </div>
                    <div className="text-right">
              ...
              <div className="bg-slate-800 rounded-xl border border-slate-700">
                <div className="p-4 bg-slate-700 border-b border-slate-600">
                  <h3 className="font-bold text-white">
                    Quarterly Milestones (FY 2025-26)
                  </h3>
                </div>
            ...
            <div className="space-y-6">
              <div className="bg-slate-800 rounded-xl border border-slate-700">
                <div className="p-4 bg-slate-700 border-b border-slate-600">
                  ...
                  <p className="text-sm text-gray-400">
                    All 15 categories -{" "}
                    {scope3Categories.filter((c) => c.is_measured).length}{" "}
                    currently measured
                  </p>
                </div>
              ...
              <div className="bg-slate-800 rounded-xl border border-slate-700">
                <div className="p-4 bg-slate-700 border-b border-slate-600 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      Monthly action schedule for Year 2
                    </p>
                  </div>
                  <button
                  ...
                  >
                    ...
                    <div className="text-center">
                      ...
                      >
                        {dq.score >= dq.target
                          ? "✓ Met"
                          : `${dq.target - dq.score} points needed`}
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="bg-slate-800 rounded-xl border border-slate-700">
                ...
                <div className="p-4 space-y-4">
                  {[
                    {
                      action:
                        "Complete fuel-card audit for 100% fleet transactions",
                      impact: "+2 points",
                      scope: "Scope 1",
                      priority: "high",
                    },
                    {
                      action:
                        "Install smart electricity meters with auto-upload",
                      impact: "+3 points",
                      scope: "Scope 2",
                      priority: "high",
                    },
                    {
                      action:
                        "Replace estimated gas readings with actual meter reads",
                      impact: "+2 points",
                      scope: "Scope 1",
                      priority: "medium",
                    },
                    {
                      action:
                        "Engage top 10 suppliers for specific emission data",
                      impact: "+2 points",
                      scope: "Scope 3",
                      priority: "medium",
                    },
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-800 rounded-xl border border-slate-700">
                  ...
                  <div className="p-4 space-y-3">
                    {[
                      {
                        item: "Electricity bills (12 months)",
                        uploaded: true,
                        verified: true,
                      },
                      {
                        item: "Gas bills (12 months)",
                        uploaded: true,
                        verified: false,
                      },
                      {
                        item: "Fleet fuel card statements",
                        uploaded: true,
                        verified: true,
                      },
                      {
                        item: "Waste transfer notes",
                        uploaded: false,
                        verified: false,
                      },
                      {
                        item: "Business travel records",
                        uploaded: true,
                        verified: false,
                      },
                      {
                        item: "Improvement plan evidence",
                        uploaded: true,
                        verified: true,
                      },
                    ].map((doc, i) => (
