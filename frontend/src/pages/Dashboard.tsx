...
import type { Incident, ExecutiveDashboardData } from "../api/client";
import {
  Card,
  CardHeader,
  CardTitle,
...
export default function Dashboard() {
  ...
  const rtaItems = data?.rtas ?? [];
  const complaintItems = data?.complaints ?? [];
  const actionItems = data?.actions ?? [];
  const auditItems = data?.auditRuns ?? [];
  const unreadCount = data?.unreadCount ?? 0;
  const execDash = (data?.execDash ?? null) as ExecutiveDashboardData | null;

  const incidents: Incident[] = useMemo(
    () => incidentItems.slice(0, 5),
    [incidentItems],
  );
  const upcomingAudits = auditItems;

  const stats: ModuleStats = useMemo(() => {
    const openIncidents = incidentItems.filter((i) => i.status !== "closed");
    const criticalIncidents = incidentItems.filter(
      (i) => i.severity === "critical" || i.severity === "high",
    );
    const openRtas = rtaItems.filter((r) => r.status !== "closed");
    const openComplaints = complaintItems.filter(
    ...
    return {
      ...
      rtas: { total: rtaItems.length, open: openRtas.length, trend: 0 },
      complaints: {
  ...
  }, [incidentItems, auditItems, complaintItems, actionItems, rtaItems]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Quality Governance Platform Overview
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <StatCard
          ...
          link="/planet-mark"
          subtitle={
            stats.carbon.totalEmissions > 0
              ? `${stats.carbon.totalEmissions} total tCO₂e`
              : undefined
          }
        />
      </div>
