...
const TELEMETRY_ENDPOINT =
  import.meta.env.VITE_TELEMETRY_ENDPOINT || "/api/v1/telemetry/web-vitals";
...
function sendMetric(metric: Metric): void {
  const payload = {
    ...
    navigationType:
      metric.navigationType ?? (performance.getEntriesByType("navigation")[0] as PerformanceNavigationTiming | undefined)?.type,
    url: location.href,
