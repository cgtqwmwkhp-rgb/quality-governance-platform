  ...
  const exchangeToken = async (
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          "[PortalAuth] Token exchange failed:",
          response.status,
          errorData,
        );
        return null;
      ...
      console.log(
        "[PortalAuth] Token exchange successful for user:",
        data.user.email,
      );
      ...
      if (idToken) {
        ...
        if (exchangeResult) {
          ...
          const newUser: PortalUser = {
            ...
            lastName:
              exchangeResult.user.full_name.split(" ").slice(1).join(" ") || "",
            jobTitle: "",
            department: "",
          };
          ...
          console.error(
            "[PortalAuth] Token exchange failed - forcing re-authentication",
          );
        ...
        if (savedUser) {
          ...
          if (sessionTime) {
            ...
            if (elapsed < 24 * 60 * 60 * 1000) {
              ...
              const storedPlatformToken = sessionStorage.getItem(
              ...
              console.log(
                "[PortalAuth] Session expired - clearing stored data",
              );
              localStorage.removeItem("portal_user");
              localStorage.removeItem("portal_session_time");
