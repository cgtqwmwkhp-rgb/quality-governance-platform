...
export {
  isSetupRequired,
  SetupRequiredPanel,
} from "../components/ui/SetupRequiredPanel";
...
export const auditsApi = {
  ...
  createSection: (templateId: number, data: AuditSectionCreate) =>
    api.post<AuditSection>(
      `/api/v1/audits/templates/${templateId}/sections`,
      data,
    ),
  ...
  createQuestion: (templateId: number, data: AuditQuestionCreate) =>
    api.post<AuditQuestion>(
      `/api/v1/audits/templates/${templateId}/questions`,
      data,
    ),
...
export const investigationsApi = {
  ...
  listSourceRecords: (
    ...
    return api.get<SourceRecordsResponse>(
      `/api/v1/investigations/source-records?${params}`,
    );
  },
  ...
  getTimeline: (
    id: number,
    options?: { page?: number; page_size?: number; type?: string },
  ) => {
    const params = new URLSearchParams();
    if (options?.page) params.set("page", String(options.page));
    ...
    return api.get<TimelineResponse>(
      `/api/v1/investigations/${id}/timeline?${params}`,
    );
  },
  ...
  getComments: (
    ...
    return api.get<CommentsResponse>(
      `/api/v1/investigations/${id}/comments?${params}`,
    );
  },
  ...
  getPacks: (id: number, options?: { page?: number; page_size?: number }) => {
    ...
    if (options?.page) params.set("page", String(options.page));
    if (options?.page_size) params.set("page_size", String(options.page_size));
    return api.get<PacksResponse>(
      `/api/v1/investigations/${id}/packs?${params}`,
    );
  },
  ...
  generatePack: (id: number, audience: string) =>
    api.post<CustomerPackSummary>(
      `/api/v1/investigations/${id}/customer-pack`,
      { audience },
    ),
  ...
  getClosureValidation: (id: number) =>
    api.get<ClosureValidation>(
      `/api/v1/investigations/${id}/closure-validation`,
    ),
};

export const standardsApi = {
  list: (page = 1, size = 10) =>
    api.get<PaginatedResponse<Standard>>(
      `/api/v1/standards/?page=${page}&page_size=${size}`,
    ),
  get: (id: number) =>
  ...
  getComplianceScore: (standardId: number) =>
    api.get<ComplianceScore>(
      `/api/v1/standards/${standardId}/compliance-score`,
    ),
};
...
export const uvdbApi = {
   ...
   */
  getProtocol: () =>
    api.get<{ sections: UVDBSection[]; total_questions: number }>(
...
export const auditTrailApi = {
  ...
  getByEntity: (entityType: string, entityId: string) =>
    api.get<AuditLogEntry[]>(
      `/api/v1/audit-trail/entity/${entityType}/${entityId}`,
    ),
...
export const signaturesApi = {
  list: (status?: string, limit = 50) => {
    ...
    return api.get<SignatureRequestEntry[]>(
      `/api/v1/signatures/requests?${sp}`,
    );
  },
  get: (id: number) =>
  ...
  send: (id: number) =>
    api.post<{ status: string; reference: string }>(
      `/api/v1/signatures/requests/${id}/send`,
    ),
  void: (id: number, reason?: string) =>
    api.post<{ status: string; reference: string }>(
      `/api/v1/signatures/requests/${id}/void`,
      { reason },
    ),
  getPending: () =>
...
export const aiApi = {
  ...
  getPredictions: (module?: string) =>
    api.get<unknown>(
      `/api/v1/ai/predictions${module ? `?module=${module}` : ""}`,
    ),
  ...
  getRecommendations: (module?: string) =>
    api.get<unknown>(
      `/api/v1/ai/recommendations${module ? `?module=${module}` : ""}`,
    ),
...
export const analyticsApi = {
  getKPIs: (timeRange?: string) =>
    api.get<unknown>(
      `/api/v1/analytics/kpis${timeRange ? `?time_range=${timeRange}` : ""}`,
    ),
  ...
  getBenchmarks: (industry?: string) =>
    api.get<unknown>(
      `/api/v1/analytics/benchmarks${industry ? `?industry=${industry}` : ""}`,
    ),
  ...
  updateDashboard: (
    id: number,
    data: { name?: string; description?: string; layout?: unknown },
  ) =>
    api.put<{ id: number; name: string }>(
      `/api/v1/analytics/dashboards/${id}`,
      data,
    ),
...
export const complianceApi = {
  ...
  listEvidenceLinks: (params?: {
    ...
    return api.get<EvidenceLinkRecord[]>(
      `/api/v1/compliance/evidence/links?${sp}`,
    );
  },
  deleteEvidenceLink: (linkId: number) =>
    api.delete<{ status: string }>(
      `/api/v1/compliance/evidence/link/${linkId}`,
    ),
...
export const complianceAutomationApi = {
  listRegulatoryUpdates: (params?: {
    ...
    if (params?.reviewed !== undefined)
      sp.set("reviewed", String(params.reviewed));
  ...
  reviewUpdate: (
    updateId: number,
    data?: { requires_action?: boolean; action_notes?: string },
  ) => {
    const sp = new URLSearchParams();
    if (data?.requires_action !== undefined)
      sp.set("requires_action", String(data.requires_action));
  ...
  runGapAnalysis: (params?: {
    ...
    return api.post<unknown>(
      `/api/v1/compliance-automation/gap-analysis/run?${sp}`,
    );
  },
  ...
  listCertificates: (params?: {
    ...
    if (params?.certificate_type)
      sp.set("certificate_type", params.certificate_type);
    if (params?.entity_type) sp.set("entity_type", params.entity_type);
  ...
  listScheduledAudits: (params?: {
    upcoming_days?: number;
    overdue?: boolean;
  }) => {
    const sp = new URLSearchParams();
    if (params?.upcoming_days)
      sp.set("upcoming_days", String(params.upcoming_days));
    if (params?.overdue !== undefined)
      sp.set("overdue", String(params.overdue));
  ...
  getComplianceScore: (params?: { scope_type?: string; scope_id?: string }) => {
    ...
    return api.get<Record<string, unknown>>(
      `/api/v1/compliance-automation/score?${sp}`,
    );
  },
  ...
  prepareRiddor: (incidentId: number, riddorType: string) => {
    ...
    return api.post<unknown>(
      `/api/v1/compliance-automation/riddor/prepare/${incidentId}?${sp}`,
    );
  },
  submitRiddor: (incidentId: number) =>
    api.post<unknown>(
      `/api/v1/compliance-automation/riddor/submit/${incidentId}`,
    ),
};
...
export const evidenceAssetsApi = {
  ...
  list: (options?: {
    ...
    if (options?.source_module)
      params.set("source_module", options.source_module);
  ...
  getSignedUrl: (assetId: number, expiresIn?: number) => {
    ...
    return api.get<SignedUrlResponse>(
      `/api/v1/evidence-assets/${assetId}/signed-url?${params}`,
    );
  },
};
...
export const executiveDashboardApi = {
  getDashboard: (periodDays = 30) =>
    api.get<ExecutiveDashboardData>(
      `/api/v1/executive-dashboard?period_days=${periodDays}`,
    ),
  getSummary: () =>
    api.get<{
      health_score: number;
