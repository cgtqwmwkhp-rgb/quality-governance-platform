...
// ============ Planet Mark Types ============
export interface PlanetMarkYear {
  id: number
  year_label: string
  year_number: number
  period: string
...
export interface PlanetMarkDashboard {
...
export const planetMarkApi = {
  getDashboard: () =>
    api.get<PlanetMarkDashboard>('/api/v1/planet-mark/dashboard'),
  listYears: () =>
    api.get<{ total: number; years: PlanetMarkYear[] }>('/api/v1/planet-mark/years'),
  getYear: (yearId: number) =>
    api.get<Record<string, unknown>>(`/api/v1/planet-mark/years/${yearId}`),
  getScope3: (yearId: number) =>
    api.get<{ year_id: number; measured_count: number; total_co2e: number; categories: Scope3Category[] }>(
      `/api/v1/planet-mark/years/${yearId}/scope3`
    ),
  getActions: (yearId: number, status?: string) =>
    api.get<{ year_id: number; summary: Record<string, number>; actions: ImprovementAction[] }>(
      `/api/v1/planet-mark/years/${yearId}/actions${status ? `?status=${status}` : ''}`
    ),
  getDataQuality: (yearId: number) =>
    api.get<Record<string, unknown>>(`/api/v1/planet-mark/years/${yearId}/data-quality`),
  getCertification: (yearId: number) =>
    api.get<Record<string, unknown>>(`/api/v1/planet-mark/years/${yearId}/certification`),
  getISO14001Mapping: () =>
    api.get<{ description: string; mappings: Array<Record<string, string>> }>('/api/v1/planet-mark/iso14001-mapping'),
}
...
export const uvdbApi = {
  getProtocol: () =>
    api.get<Record<string, unknown>>('/api/v1/uvdb/protocol'),
  listSections: () =>
    api.get<{ total_sections: number; sections: UVDBSection[] }>('/api/v1/uvdb/sections'),
  getSectionQuestions: (sectionNumber: string) =>
    api.get<Record<string, unknown>>(`/api/v1/uvdb/sections/${sectionNumber}/questions`),
  listAudits: (skip = 0, limit = 50, status?: string, companyName?: string) => {
    const params = new URLSearchParams()
    params.set('skip', skip.toString())
    params.set('limit', limit.toString())
    if (status) params.set('status', status)
    if (companyName) params.set('company_name', companyName)
    return api.get<{ total: number; audits: UVDBAudit[] }>(`/api/v1/uvdb/audits?${params.toString()}`)
  },
  getAudit: (auditId: number) =>
    api.get<Record<string, unknown>>(`/api/v1/uvdb/audits/${auditId}`),
  getDashboard: () =>
    api.get<UVDBDashboard>('/api/v1/uvdb/dashboard'),
  getISOMapping: () =>
    api.get<{ description: string; total_mappings: number; mappings: Array<Record<string, unknown>>; summary: Record<string, string> }>(
      '/api/v1/uvdb/iso-mapping'
    ),
}
