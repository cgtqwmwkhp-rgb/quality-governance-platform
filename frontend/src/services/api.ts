...
export const formTemplatesApi = {
  ...
  delete: (id: number) => apiRequest<void>(`/admin/config/templates/${id}`, { method: 'DELETE' }),
}
...
export const contractsApi = {
  ...
  delete: (id: number) => apiRequest<void>(`/admin/config/contracts/${id}`, { method: 'DELETE' }),
}
...
export const settingsApi = {
  ...
  get: (key: string) => apiRequest<SystemSetting>(`/admin/config/settings/${key}`, { cache: true }),
...
export const nearMissApi = {
  ...
  delete: (id: number) => apiRequest<void>(`/near-misses/${id}`, { method: 'DELETE' }),
}
...
export const rtasApi = {
  create: (data: RTACreate) => apiRequest<RTAResponse>('/rtas/', { method: 'POST', body: data }),

  list: (page = 1, pageSize = 20, status?: string) => {
    const params = new URLSearchParams({ page: String(page), page_size: String(pageSize) })
    if (status) params.append('status', status)
    return apiRequest<{ items: RTAResponse[]; total: number }>(`/rtas/?${params.toString()}`)
  },
