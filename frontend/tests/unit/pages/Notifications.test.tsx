...
vi.mock('../../../src/api/client', () => ({
  ...
  notificationsApi: { getUnreadCount: vi.fn().mockResolvedValue({ data: { unread_count: 0 } }), list: vi.fn().mockResolvedValue({ items: [] }), markRead: vi.fn().mockResolvedValue({}), markAllRead: vi.fn().mockResolvedValue({}), delete: vi.fn().mockResolvedValue({}) },
...
describe('Notifications', () => {
  it('renders without crashing', async () => {
    ...
    const subtitle = await screen.findByText('Stay updated with important alerts and reminders');
