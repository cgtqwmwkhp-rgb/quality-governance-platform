...
vi.mock('../../../src/api/client', () => ({
  ...
  usersApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0, page: 1, size: 10, pages: 0 } }), listRoles: vi.fn().mockResolvedValue({ data: [] }) },
...
describe('UserManagement', () => {
  it('renders without crashing', async () => {
    ...
    const heading = await screen.findByText('User Management');
  ...
  it('renders the Add User button', async () => {
    ...
    const btn = await screen.findByText('Add User');
    expect(btn).toBeTruthy();
  });

  it('renders the Users and Roles tabs', async () => {
    render(
      <MemoryRouter>
        <UserManagement />
      </MemoryRouter>
