...
vi.mock('../../../src/api/client', () => ({
  ...
  actionsApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0, page: 1, size: 10, pages: 0 } }), create: vi.fn().mockResolvedValue({ data: {} }), update: vi.fn().mockResolvedValue({ data: {} }) },
  ...
  investigationsApi: { list: vi.fn().mockResolvedValue({ data: { items: [], total: 0, page: 1, size: 10, pages: 0 } }), listSourceRecords: vi.fn().mockResolvedValue({ data: { items: [] } }), createFromRecord: vi.fn().mockResolvedValue({ data: {} }) },
...
describe('Investigations', () => {
  it('renders without crashing', async () => {
    ...
    const heading = await screen.findByText('Root Cause Investigations');
