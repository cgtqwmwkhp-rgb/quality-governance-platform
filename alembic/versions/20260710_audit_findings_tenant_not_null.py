"""Fail-safe backfill + conditional NOT NULL for audit_findings.tenant_id.

Revision ID: 20260710_af_tenant_nn
Revises: 20260710_doc_ctl_tenant
Create Date: 2026-07-10

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
parent audit_runs row via run_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. parent run still
NULL).
"""
...
revision: str = "20260710_af_tenant_nn"
...
def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every audit_findings row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    """Copy tenant_id from audit_runs where the parent is already attributed.

    Does not invent a default tenant. Rows whose parent run.tenant_id is NULL
...
def upgrade() -> None:
    ...
    msg = (
        ...
        "Likely parent audit_runs.tenant_id is still NULL; do not invent tenant_id=1. "
