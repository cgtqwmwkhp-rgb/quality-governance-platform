"""Fail-safe backfill + conditional NOT NULL for complaint_actions.tenant_id.

Revision ID: 20260710_ca_tenant_nn
Revises: 20260710_ia_tenant_nn
Create Date: 2026-07-10

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
parent complaints row via complaint_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. parent complaint
still NULL).
"""
...
revision: str = "20260710_ca_tenant_nn"
down_revision: Union[str, Sequence[str], None] = "20260710_ia_tenant_nn"
...
PARENT = "complaints"
PARENT_KEY = "complaint_id"


def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every complaint_actions row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    """Copy tenant_id from complaints where the parent is already attributed.

    Does not invent a default tenant. Rows whose parent complaint.tenant_id is NULL
...
def upgrade() -> None:
    ...
    msg = (
        ...
        "Likely parent complaints.tenant_id is still NULL; do not invent tenant_id=1. "
