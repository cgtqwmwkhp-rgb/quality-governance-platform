"""Fail-safe backfill + conditional NOT NULL for external_audit_import_drafts.tenant_id.

Revision ID: 20260710_ext_draft_nn
Revises: 20260710_ext_audit_tenant_nn
Create Date: 2026-07-10

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
parent external_audit_import_jobs row via import_job_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. parent job
still NULL).
"""
...
revision: str = "20260710_ext_draft_nn"
down_revision: Union[str, Sequence[str], None] = "20260710_ext_audit_tenant_nn"
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None

logger = logging.getLogger("alembic.runtime.migration")

TABLE = "external_audit_import_drafts"
PARENT = "external_audit_import_jobs"
PARENT_KEY = "import_job_id"


def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every external_audit_import_drafts row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    """Copy tenant_id from import jobs where the parent is already attributed.

    Does not invent a default tenant. Rows whose parent job.tenant_id is NULL
...
def upgrade() -> None:
    ...
    msg = (
        ...
        "Likely parent external_audit_import_jobs.tenant_id is still NULL; do not invent tenant_id=1. "
