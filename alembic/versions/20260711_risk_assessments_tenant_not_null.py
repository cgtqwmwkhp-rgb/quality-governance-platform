"""Fail-safe backfill + conditional NOT NULL for risk_assessments.tenant_id.

WCS-TEN2 next table after RTA parent: inherit from risks via risk_id.

Revision ID: 20260711_ra_tenant_nn
Revises: 20260711_rta_parent_nn
Create Date: 2026-07-11

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
parent risks row via risk_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. parent risk still
NULL).
"""
...
revision: str = "20260711_ra_tenant_nn"
down_revision: Union[str, Sequence[str], None] = "20260711_rta_parent_nn"
...
def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every risk_assessments row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    ...
    Does not invent a default tenant. Rows whose parent risk.tenant_id is NULL
...
def _align_mismatches_to_parent() -> None:
    ...
    Only rewrites when the parent risk has a non-NULL tenant_id. Documented
...
def upgrade() -> None:
    ...
    msg = (
        ...
        "Likely parent risks.tenant_id is still NULL; do not invent tenant_id=1. "
