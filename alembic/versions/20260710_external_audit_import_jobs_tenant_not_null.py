...
parent audit_runs row via audit_run_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. parent audit run
still NULL).
"""
...
TABLE = "external_audit_import_jobs"
PARENT = "audit_runs"
PARENT_KEY = "audit_run_id"


def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every external_audit_import_jobs row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    ...
    Does not invent a default tenant. Rows whose parent audit run.tenant_id is NULL
