"""Fail-safe backfill + conditional NOT NULL for controlled_documents.tenant_id.

WCS-TEN2: inherit controlled_documents.tenant_id from users via author_id, then owner_id.

Revision ID: 20260711_cd_tenant_nn
Revises: 20260711_cdv_tenant_nn
Create Date: 2026-07-11

WCS-TEN2 / C-01 Phase 2 (PARENT core): inherit tenant_id from the author
user via author_id, then owner_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows.
"""
...
revision: str = "20260711_cd_tenant_nn"
down_revision: Union[str, Sequence[str], None] = "20260711_cdv_tenant_nn"
...
def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every controlled_documents row has a tenant_id."""
...
def upgrade() -> None:
    ...
    if _table_exists(PARENT) and _has_column(PARENT, "tenant_id") and _has_column(TABLE, "owner_id"):
        _backfill_from_user_fk("owner_id")
    ...
    msg = (
        ...
        f"{remaining} row(s) still have tenant_id IS NULL after author/owner backfill. "
        "Likely author/owner users.tenant_id is still NULL or FKs are unset; "
