"""Fail-safe backfill + conditional NOT NULL for incident_actions.tenant_id.

Revision ID: 20260710_ia_tenant_nn
Revises: 20260710_af_tenant_nn
Create Date: 2026-07-10

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
parent incidents row via incident_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. parent incident still
NULL).
"""
...
revision: str = "20260710_ia_tenant_nn"
down_revision: Union[str, Sequence[str], None] = ("20260710_af_tenant_nn", "20260710_audit_meta")
...
def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every incident_actions row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    """Copy tenant_id from incidents where the parent is already attributed.

    Does not invent a default tenant. Rows whose parent incident.tenant_id is NULL
...
def _align_mismatches_to_parent() -> None:
    """Prefer parent ownership when child.tenant_id differs from run.tenant_id.

    Only rewrites when the parent run has a non-NULL tenant_id. Documented
...
def upgrade() -> None:
    ...
    msg = (
        ...
        "Likely parent incidents.tenant_id is still NULL; do not invent tenant_id=1. "
