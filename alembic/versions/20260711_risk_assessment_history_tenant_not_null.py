"""Fail-safe backfill + conditional NOT NULL for risk_assessment_history.tenant_id.

WCS-TEN2: inherit risk_assessment_history.tenant_id from risks_v2 via risk_id.

Revision ID: 20260711_rah_tenant_nn
Revises: 20260711_bow_tenant_nn
Create Date: 2026-07-11

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
parent risks_v2 row via risk_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. parent risks_v2
row still NULL).
"""
...
revision: str = "20260711_rah_tenant_nn"
down_revision: Union[str, Sequence[str], None] = "20260711_kri_tenant_nn"
...
def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every risk_assessment_history row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    """Copy tenant_id from parent risks_v2 where the parent is already attributed.

    Does not invent a default tenant. Rows whose parent risks_v2.tenant_id is NULL
...
def _align_mismatches_to_parent() -> None:
    ...
    Only rewrites when the parent risks_v2 row has a non-NULL tenant_id. Documented
...
def upgrade() -> None:
    ...
    msg = (
        ...
        "Likely parent risks_v2.tenant_id is still NULL; do not invent tenant_id=1. "
