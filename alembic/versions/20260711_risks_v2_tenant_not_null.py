"""Fail-safe backfill + conditional NOT NULL for risks_v2.tenant_id.

WCS-TEN2: inherit risks_v2.tenant_id from users via created_by then risk_owner_id.

Revision ID: 20260711_rv2_tenant_nn
Revises: 20260711_ra_tenant_nn
Create Date: 2026-07-11

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
creator users row via risk_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. creator user still
NULL).
"""
...
revision: str = "20260711_rv2_tenant_nn"
down_revision: Union[str, Sequence[str], None] = "20260711_ra_tenant_nn"
...
TABLE = "risks_v2"
PARENT = "users"
PARENT_KEY = "created_by"


def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every risks_v2 row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    """Copy tenant_id from risks where the parent is already attributed.
...
def _align_mismatches_to_parent() -> None:
    ...
    Only rewrites when the creator user has a non-NULL tenant_id. Documented
...
def upgrade() -> None:
    ...
    if _table_exists(PARENT) and _has_column(PARENT, "tenant_id") and _has_column(TABLE, "risk_owner_id"):
        op.execute(
            sa.text(
                f"""
                UPDATE {TABLE}
                SET tenant_id = (
                    SELECT parent.tenant_id
                    FROM {PARENT} AS parent
                    WHERE parent.id = {TABLE}.risk_owner_id
                )
                WHERE tenant_id IS NULL
                  AND EXISTS (
                    SELECT 1
