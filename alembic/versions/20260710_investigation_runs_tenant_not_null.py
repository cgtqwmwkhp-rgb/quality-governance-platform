...
parent investigation_templates row via template_id. Never invent tenant_id=1.
...
PARENT = "investigation_templates"
PARENT_KEY = "template_id"


def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every investigation_runs row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    """Copy tenant_id from investigation_templates where the parent is already attributed.
...
def upgrade() -> None:
    ...
    msg = (
        ...
        "Likely parent investigation_templates.tenant_id is still NULL; do not invent tenant_id=1. "
