"""Fail-safe backfill + conditional NOT NULL for rta_actions.tenant_id.

Revision ID: 20260710_rta_act_nn
Revises: 20260710_ar_tenant_nn
Create Date: 2026-07-10

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
parent road_traffic_collisions row via rta_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. parent RTA
still NULL).
"""
...
revision: str = "20260710_rta_act_nn"
down_revision: Union[str, Sequence[str], None] = "20260710_ar_tenant_nn"
...
TABLE = "rta_actions"
PARENT = "road_traffic_collisions"
PARENT_KEY = "rta_id"


def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every rta_actions row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    """Copy tenant_id from road_traffic_collisions where the parent is already attributed.

    Does not invent a default tenant. Rows whose parent RTA.tenant_id is NULL
...
def upgrade() -> None:
    ...
    msg = (
        ...
        "Likely parent road_traffic_collisions.tenant_id is still NULL; do not invent tenant_id=1. "
