"""Fail-safe backfill + conditional NOT NULL for enterprise_risk_controls.tenant_id.

WCS-TEN2: inherit enterprise_risk_controls.tenant_id from users via control_owner_id.

Revision ID: 20260711_erc_tenant_nn
Revises: 20260711_bow_tenant_nn
Create Date: 2026-07-11

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
control owner users row via control_owner_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. control owner users
row still NULL).
"""
...
revision: str = "20260711_erc_tenant_nn"
down_revision: Union[str, Sequence[str], None] = "20260711_rah_tenant_nn"
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None

logger = logging.getLogger("alembic.runtime.migration")

TABLE = "enterprise_risk_controls"
PARENT = "users"
PARENT_KEY = "control_owner_id"


def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every enterprise_risk_controls row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    """Copy tenant_id from control owner users where the parent is already attributed.

    Does not invent a default tenant. Rows whose control owner users.tenant_id is NULL
    remain NULL until the parent is attributed.
    """
...
def _align_mismatches_to_parent() -> None:
    """Prefer parent ownership when child.tenant_id differs from risk.tenant_id.

    Only rewrites when the control owner users row has a non-NULL tenant_id. Documented
...
def upgrade() -> None:
    if not _table_exists(TABLE) or not _has_column(TABLE, "tenant_id"):
        msg = f"Skipping {revision}: {TABLE}.tenant_id missing " "(nothing to backfill / constrain)."
        logger.warning(msg)
        print(msg)
        return
    ...
    msg = (
        ...
        "Likely control owner users.tenant_id is still NULL; do not invent tenant_id=1. "
