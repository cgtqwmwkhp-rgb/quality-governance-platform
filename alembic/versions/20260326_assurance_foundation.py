...
def _table_exists(table_name: str) -> bool:
    bind = op.get_bind()
    return bind.execute(text("SELECT to_regclass(:table_name)"), {"table_name": table_name}).scalar() is not None
...
def downgrade() -> None:
    if _table_exists("cross_standard_mappings"):
        ...
        op.execute("ALTER TABLE cross_standard_mappings DROP CONSTRAINT IF EXISTS fk_cross_standard_mapped_clause")
        op.execute("ALTER TABLE cross_standard_mappings DROP CONSTRAINT IF EXISTS fk_cross_standard_primary_clause")
    ...
    if _table_exists("certificates"):
        ...
        op.execute("ALTER TABLE certificates DROP CONSTRAINT IF EXISTS fk_certificates_primary_evidence_asset")
    ...
    if _table_exists("audit_runs"):
        ...
        op.execute("ALTER TABLE audit_runs DROP CONSTRAINT IF EXISTS fk_audit_runs_source_document_asset")
