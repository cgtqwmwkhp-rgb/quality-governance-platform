"""Fail-safe backfill + conditional NOT NULL for controlled_document_versions.tenant_id.

WCS-TEN2: inherit controlled_document_versions.tenant_id from controlled_documents via document_id.

Revision ID: 20260711_cdv_tenant_nn
Revises: 20260711_pv_tenant_nn
Create Date: 2026-07-11

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
parent controlled_documents row via document_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. parent controlled_documents
row still NULL).
"""
...
revision: str = "20260711_cdv_tenant_nn"
down_revision: Union[str, Sequence[str], None] = "20260711_pv_tenant_nn"
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None

logger = logging.getLogger("alembic.runtime.migration")

TABLE = "controlled_document_versions"
PARENT = "controlled_documents"
PARENT_KEY = "document_id"


def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every controlled_document_versions row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    """Copy tenant_id from parent controlled_documents where attributed.

    Does not invent a default tenant. Rows whose parent tenant_id is NULL
...
def _align_mismatches_to_parent() -> None:
    ...
    Only rewrites when the parent policies row has a non-NULL tenant_id. Documented
...
def upgrade() -> None:
    ...
    msg = (
        ...
        "Likely parent policies.tenant_id is still NULL; do not invent tenant_id=1. "
