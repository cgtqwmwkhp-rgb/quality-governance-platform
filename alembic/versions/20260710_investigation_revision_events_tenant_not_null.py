"""Fail-safe backfill + conditional NOT NULL for investigation_revision_events.tenant_id.

Revision ID: 20260710_inv_rev_evt_nn
Revises: 20260710_inv_cmt_nn
Create Date: 2026-07-10

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
parent investigation_runs row via investigation_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. parent investigation run
still NULL).
"""
...
revision: str = "20260710_inv_rev_evt_nn"
down_revision: Union[str, Sequence[str], None] = "20260710_inv_cmt_nn"
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None

logger = logging.getLogger("alembic.runtime.migration")

TABLE = "investigation_revision_events"
PARENT = "investigation_runs"
PARENT_KEY = "investigation_id"


def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every investigation_revision_events row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    """Copy tenant_id from investigation_runs where the parent is already attributed.

    Does not invent a default tenant. Rows whose parent investigation run.tenant_id is NULL
...
def upgrade() -> None:
    ...
    msg = (
        ...
        "Likely parent investigation_runs.tenant_id is still NULL; do not invent tenant_id=1. "
