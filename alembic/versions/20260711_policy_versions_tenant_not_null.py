"""Fail-safe backfill + conditional NOT NULL for policy_versions.tenant_id.

WCS-TEN2: inherit policy_versions.tenant_id from policies via policy_id.

Revision ID: 20260711_pv_tenant_nn
Revises: 20260711_bow_tenant_nn
Create Date: 2026-07-11

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
parent policies row via policy_id. Never invent tenant_id=1.
...
deploys are not blocked by residual unattributed rows (e.g. parent policies
row still NULL).
"""
...
revision: str = "20260711_pv_tenant_nn"
down_revision: Union[str, Sequence[str], None] = "20260711_erc_tenant_nn"
...
def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every policy_versions row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    """Copy tenant_id from parent policies where the parent is already attributed.

    Does not invent a default tenant. Rows whose parent policies.tenant_id is NULL
