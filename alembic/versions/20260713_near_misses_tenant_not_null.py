"""Fail-safe backfill + conditional NOT NULL for near_misses.tenant_id.

Revision ID: 20260713_nm_tenant_nn
Revises: 20260712_capa_src_check
Create Date: 2026-07-13

WCS-TEN2 / C-01 Phase 2 (PARENT core): inherit tenant_id from the creator
user via created_by_id. Never invent tenant_id=1.

near_misses is a parent core Safety CUJ entity (sibling to incidents and
road_traffic_collisions). The most reliable attribution path for legacy NULL
rows is through the creating user's tenant membership (same pattern as
incidents).
...
revision: str = "20260713_nm_tenant_nn"
down_revision: Union[str, Sequence[str], None] = "20260712_capa_src_check"
...
def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every near_misses row has a tenant_id."""
...
def _align_mismatches_to_parent() -> None:
    """Prefer creator ownership when near_miss.tenant_id differs from creator.tenant_id.
