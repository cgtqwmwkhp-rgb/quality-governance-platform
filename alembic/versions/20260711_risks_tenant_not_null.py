"""Fail-safe backfill + conditional NOT NULL for risks.tenant_id.

Revision ID: 20260711_risks_tenant_nn
Revises: 20260710_cmp_tenant_nn
...
revision: str = "20260711_risks_tenant_nn"
down_revision: Union[str, Sequence[str], None] = "20260710_cmp_tenant_nn"
...
def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every risks row has a tenant_id."""
...
def _align_mismatches_to_parent() -> None:
    """Prefer creator ownership when risk.tenant_id differs from creator.tenant_id."""
    op.execute(
        sa.text(
            f"""
            UPDATE {TABLE}
