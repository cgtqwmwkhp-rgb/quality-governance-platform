"""Fail-safe backfill + conditional NOT NULL for policies.tenant_id.

WCS-TEN2: inherit policies.tenant_id from users via created_by_id, then owner_id.

Revision ID: 20260711_pol_tenant_nn
Revises: 20260711_cd_tenant_nn
Create Date: 2026-07-11

WCS-TEN2 / C-01 Phase 2 (PARENT core): inherit tenant_id from the creator
user via created_by_id, then owner_id. Never invent tenant_id=1.
...
revision: str = "20260711_pol_tenant_nn"
down_revision: Union[str, Sequence[str], None] = "20260711_cd_tenant_nn"
...
def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every policies row has a tenant_id."""
...
def _align_mismatches_to_user_fk(fk_column: str) -> None:
    """Prefer linked-user ownership when policy.tenant_id differs from user.tenant_id."""
    op.execute(
        sa.text(
            f"""
            UPDATE {TABLE}
...
def upgrade() -> None:
    ...
    msg = (
        ...
        f"{remaining} row(s) still have tenant_id IS NULL after creator/owner backfill. "
        "Likely creator/owner users.tenant_id is still NULL or FKs are unset; "
