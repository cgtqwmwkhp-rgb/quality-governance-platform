"""Fail-safe backfill + conditional NOT NULL for capa_actions.tenant_id.

Revision ID: 20260710_capa_act_nn
Revises: 20260710_rta_act_nn
Create Date: 2026-07-10

WCS-TEN2 / C-01 Phase 2 (single table family): inherit tenant_id from the
creator user row via created_by_id. Never invent tenant_id=1.

capa_actions is a standalone CAPA record (no single parent FK like
incident_actions or complaint_actions). The most reliable attribution path
is through the creating user's tenant membership.
...
revision: str = "20260710_capa_act_nn"
down_revision: Union[str, Sequence[str], None] = "20260710_rta_act_nn"
...
def should_enforce_not_null(remaining_null_count: int) -> bool:
    """Return True only when every capa_actions row has a tenant_id."""
...
def _backfill_from_parent() -> None:
    ...
    remain NULL until the user is attributed.
    """
    op.execute(
        sa.text(
            f"""
...
def _align_mismatches_to_parent() -> None:
    """Prefer creator ownership when child.tenant_id differs from creator.tenant_id.

    Only rewrites when the creator has a non-NULL tenant_id. Documented
    policy for Phase 2: child inherits creator; do not invent tenants.
    """
    op.execute(
        sa.text(
            f"""
...
def upgrade() -> None:
    ...
    msg = (
        ...
        "Likely creator users.tenant_id is still NULL; do not invent tenant_id=1. "
