...
def load_baseline(baseline_path: Path) -> BaselineConfig:
    ...
    required_fields = [
        'baseline_pass_count', 'baseline_skip_count', 'baseline_total_count',
        'baseline_commit_sha', 'baseline_date', 'baseline_notes',
        'version', 'min_acceptable_percentage'
    ]
...
def validate_gate(current_passed: int, current_skipped: int, baseline_path: Path) -> bool:
    ...
    if override:
        ...
        print(f"   MIN_ACCEPTABLE = {baseline.baseline_pass_count} × {baseline.min_acceptable_percentage}% = {min_acceptable}")
    print()
