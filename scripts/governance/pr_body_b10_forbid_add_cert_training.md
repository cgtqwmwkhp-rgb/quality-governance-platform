# Change Ledger (CL-B10-FORBID-ADD-CERT-TRAINING)
...
- **Frontend (routes/screens/components):** None — no in-repo FE client posts to these auditor-competence certification/training endpoints today
- **Backend (handlers/services):** None beyond schema config
- **APIs (endpoints changed/added):** Behaviour change only for unknown fields on `POST /api/v1/auditor-competence/profiles/{user_id}/certifications` and `POST /api/v1/auditor-competence/profiles/{user_id}/training` (now 422 instead of silent drop)
- **Schemas/contracts (OpenAPI/Zod/DTO/types):** `AddCertificationRequest`, `AddTrainingRequest` → `additionalProperties: false`
...
- [x] Build — N/A
- [x] Unit tests — `tests/unit/test_add_certification_request_extra_forbid.py`, `tests/unit/test_add_training_request_extra_forbid.py`, `tests/unit/test_write_schema_extra_forbid_ratchet.py` (15 local pass)
...
- [x] CUJ-01: Certification create with only declared fields still validates; an extra field (e.g. `user_id`) raises ValidationError
- [x] CUJ-02: Training create with only declared fields still validates; an extra field (e.g. `completed`) raises ValidationError
...
- **Staging verification:** Confirm certification/training create still succeed with normal payloads; optional probe of unknown field → 422
- **Canary plan:** N/A
- **Prod post-deploy checks:** Spot-check auditor-competence certification and training create if exercised
...
## Measured baseline (`origin/main` @ `83c04b7f`)
| Metric | Before | After |
| --- | ---: | ---: |
...
- **#1455 still open** (`AcknowledgementCreate`, `AcknowledgementAction`) — this PR deliberately avoids those schemas and avoids `driver_profile.py` to reduce merge conflict surface. Baseline/inventory will need a rebase bump (6→8 / 290→288) when #1455 lands after this (or vice versa).
...
- **B-10:** inventory ratchet already on main (#1452); #1454 merged (`ActionOwnerNoteCreate`, `AddWhyRequest`). These two auditor-competence schemas are fixed-field create bodies, low blast radius, and have no in-repo FE client posting extra keys. Skipped `AddCommentRequest` (body/content alias compatibility) and large create/update aggregates.
