# Change Ledger (CL-B10-ACCESS-CONTROL-ACK-REQUIREMENT)

## 1) Summary
- **Feature / Change name:** Board B-10 (`w4-extra-forbid`) — convert two write schemas to `extra="forbid"`
- **User goal (1–2 lines):** Stop `AccessControlCreate` and `AcknowledgmentRequirementCreate` from silently ignoring unknown body fields (PX-168 class), with unit regression locks and a tightened inventory ratchet.
- **Depends on:** #1489 (CAPAStatusTransition / DefectCAPARequest pair)
- **In scope:** `ConfigDict(extra="forbid")` on those two schemas; remove them from `KNOWN_LAX_WRITE_SCHEMAS`; refresh B-10 baseline/inventory (forbid floor 34→36, open ceiling 262→260); unit tests for unknown-field rejection
...
- **APIs (endpoints changed/added):** Behaviour change only for unknown fields on `POST /api/v1/iso27001/access-control` and `POST /api/v1/policy-acknowledgments/requirements` (now 422 instead of silent drop)
- **Schemas/contracts (OpenAPI/Zod/DTO/types):** `AccessControlCreate`, `AcknowledgmentRequirementCreate` → `additionalProperties: false`
...
- [x] AC-01: `AccessControlCreate` declares `extra="forbid"` and rejects an unknown field in unit tests
- [x] AC-02: `AcknowledgmentRequirementCreate` declares `extra="forbid"` and rejects an unknown field in unit tests
- [x] AC-03: Both schemas removed from `KNOWN_LAX_WRITE_SCHEMAS`
- [x] AC-04: B-10 ratchet baseline refreshed — **forbid ≥ 36**, **open ≤ 260**
...
- [x] Build — N/A
- [x] Unit tests — `tests/unit/test_access_control_create_extra_forbid.py`, `tests/unit/test_acknowledgment_requirement_create_extra_forbid.py`, ratchet test updated
...
- [x] CUJ-01: ISO 27001 access-control create with only declared fields still validates; an extra field raises ValidationError
- [x] CUJ-02: Acknowledgment requirement create with only declared fields still validates; an extra field raises ValidationError
...
## Measured baseline (post-#1489)
| Metric | Before | After |
| --- | ---: | ---: |
| `min_forbid_count` | 34 | 36 |
| `max_open_count` | 262 | 260 |
| Schemas converted | — | `AccessControlCreate`, `AcknowledgmentRequirementCreate` |
