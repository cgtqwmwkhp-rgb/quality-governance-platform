...
- [x] Registry guards updated, not loosened: the single-head literal in `test_job_lifecycle_ux_w4/w5.py` now names `20260901_comp_cov`; the new `locations → competence_coverage_quotas` cascade is recorded in `test_delete_cascade_audit_visibility.py` (87 no-relationship pairs); the Library F-3 anti-dupe gate's `coverage_twin_table_allowlist` gains `competence_coverage_quotas` with owner and reason — headcount cover holds no standard, clause, control or evidence link, so CEL stays the coverage SoT; `SERVER_OWNED_FIELDS` records the four coverage overlay fields so a client cannot assert n-of-m on `RequirementCreate`/`Update`.
