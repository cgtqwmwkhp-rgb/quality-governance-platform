# Change Ledger (CL-B10-ASSET-TYPE-CREATE-UPDATE)

## 1) Summary
- **Feature / Change name:** Board B-10 (`w4-extra-forbid`) — convert two write schemas to `extra="forbid"`
- **User goal (1–2 lines):** Stop `AssetTypeCreate` and `AssetTypeUpdate` from silently ignoring unknown body fields (PX-168 class), with unit regression locks and a tightened inventory ratchet.
- **Depends on:** #1496 (AssessmentRunUpdate / AssessorGuidanceRequest pair)
- **In scope:** `ConfigDict(extra="forbid")` on those two schemas; remove them from `KNOWN_LAX_WRITE_SCHEMAS`; refresh B-10 baseline/inventory (forbid floor 48→50, open ceiling 248→246); unit tests for unknown-field rejection
...
- **APIs (endpoints changed/added):** Behaviour change only for unknown fields on `POST /api/v1/assets/asset-types` and `PATCH /api/v1/assets/asset-types/{asset_type_id}` (now 422 instead of silent drop)
- **Schemas/contracts (OpenAPI/Zod/DTO/types):** `AssetTypeCreate`, `AssetTypeUpdate` → `additionalProperties: false`
...
- [x] AC-01: `AssetTypeCreate` declares `extra="forbid"` and rejects an unknown field in unit tests
- [x] AC-02: `AssetTypeUpdate` declares `extra="forbid"` and rejects an unknown field in unit tests
- [x] AC-03: Both schemas removed from `KNOWN_LAX_WRITE_SCHEMAS`
- [x] AC-04: B-10 ratchet baseline refreshed — **forbid ≥ 50**, **open ≤ 246**
...
- [x] Build — N/A
- [x] Unit tests — `tests/unit/test_asset_type_create_extra_forbid.py`, `tests/unit/test_asset_type_update_extra_forbid.py`, ratchet test updated
...
- [x] CUJ-01: Asset type create with only declared fields still validates; an extra field raises ValidationError
- [x] CUJ-02: Asset type update with only declared fields still validates; an extra field raises ValidationError
...
## Measured baseline (post-#1496)
| Metric | Before | After |
| --- | ---: | ---: |
| `min_forbid_count` | 48 | 50 |
| `max_open_count` | 248 | 246 |
| Schemas converted | — | `AssetTypeCreate`, `AssetTypeUpdate` |
