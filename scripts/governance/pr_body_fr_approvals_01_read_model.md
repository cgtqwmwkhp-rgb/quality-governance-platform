...
- [x] Frontend tests — 17 new tests: `approvalsClient.test.ts` (9) and `NeedsMyDecisionPanel.test.tsx` (8, one per rendered state including "empty but unreadable" and the failure/retry path). `Actions.test.tsx` + `Actions.complianceSource.test.tsx` **43 passed** with the panel mounted. Full `vitest run` executed.
