"""Unit tests for investigation_runs tenant_id fail-safe backfill (WCS-TEN2)."""
...
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260710_investigation_runs_tenant_not_null.py"
...
def test_investigation_run_orm_tenant_id_is_required():
    column = InvestigationRun.__table__.c.tenant_id
...
def test_migration_chains_from_inv_rev_evt_and_never_invents_tenant():
    ...
    assert "investigation_templates" in body
    assert "tenant_id = 1" not in body
    assert "SET tenant_id = 1" not in body.upper()
    assert (REPO_ROOT / "docs/data/investigation-runs-tenant-backfill.md").is_file()
