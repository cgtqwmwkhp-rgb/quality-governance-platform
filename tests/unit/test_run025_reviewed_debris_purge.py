...
def test_the_reviewed_set_is_exactly_the_four_rows_that_were_measured():
    """Pinned deliberately.

    These four primary keys are the whole authority for a production delete. If a
    later change adds a fifth row, moves an id, or points a row at a different
    ...
    assert {row.creator_email for row in REVIEWED_DEBRIS} == {"smoke-runner@plantexpand.com"}
...
async def test_the_clean_case_plans_a_delete_with_children_first(debris_db):
    """``audit_findings.run_id`` is CASCADE, so deleting the run first would appear to
    work. Deleting the finding first means each statement removes exactly the row that
    was reviewed, and the operation never depends on a cascade firing."""
    debris_db.seed()
    ...
    assert set(order) == {"audit_runs#5", "audit_runs#6", "audit_findings#4", "risks_v2#2"}
    ...
    assert [(hit["parent"], hit["child"], hit["on_delete"]) for hit in inside] == [
...
async def test_a_junction_row_outside_the_four_stops_the_delete(debris_db):
    ...
    assert any("outside the reviewed four" in blocker for blocker in result["blockers"])
...
@pytest.mark.anyio
async def test_all_four_absent_on_a_dialect_with_no_rls_is_a_clean_nothing_to_do(debris_db):
    debris_db.seed()
...
def test_the_manifest_records_every_column_of_every_doomed_row(debris_db, capsys, tmp_path):
    ...
    assert set(recorded["rows"]) == {"audit_runs", "audit_findings", "risks_v2"}
...
def test_apply_deletes_exactly_the_four_and_is_then_idempotent(debris_db, capsys, tmp_path):
    debris_db.seed()
    ...
    assert applied["deleted"] == {"audit_findings": 1, "audit_runs": 2, "risks_v2": 1}
...
_ORDER: list[tuple[str, int]] = [("audit_findings", 4), ("audit_runs", 5), ("risks_v2", 2), ("audit_runs", 6)]
