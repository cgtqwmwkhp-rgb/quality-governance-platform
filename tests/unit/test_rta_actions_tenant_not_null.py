"""Unit tests for rta_actions tenant_id fail-safe backfill (WCS-TEN2)."""
...
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260710_rta_actions_tenant_not_null.py"
...
def test_rta_action_orm_tenant_id_is_required():
    column = RTAAction.__table__.c.tenant_id
...
def test_migration_chains_from_audit_runs_and_never_invents_tenant():
    ...
    assert 'revision: str = "20260710_rta_act_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = "20260710_ar_tenant_nn"' in body
    assert 'TABLE = "rta_actions"' in body
    assert 'PARENT = "road_traffic_collisions"' in body
    assert 'PARENT_KEY = "rta_id"' in body
    assert "FAIL-SAFE" in body
    assert "tenant_id = 1" not in body
    assert "SET tenant_id = 1" not in body.upper()
    assert (REPO_ROOT / "docs/data/rta-actions-tenant-backfill.md").is_file()
