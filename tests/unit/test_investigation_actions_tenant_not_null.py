"""Unit tests for investigation_actions tenant_id fail-safe backfill (WCS-TEN2)."""
...
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260710_investigation_actions_tenant_not_null.py"
...
def test_investigation_action_orm_tenant_id_is_required():
    column = InvestigationAction.__table__.c.tenant_id
...
def test_migration_chains_from_capa_actions_and_never_invents_tenant():
    body = MIGRATION_PATH.read_text(encoding="utf-8")
    assert 'revision: str = "20260710_inv_act_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = "20260710_capa_act_nn"' in body
    assert 'TABLE = "investigation_actions"' in body
    ...
    assert (REPO_ROOT / "docs/data/investigation-actions-tenant-backfill.md").is_file()
