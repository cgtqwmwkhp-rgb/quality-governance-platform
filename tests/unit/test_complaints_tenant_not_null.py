"""Unit tests for complaints tenant_id fail-safe backfill (WCS-TEN2)."""
...
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260710_complaints_tenant_not_null.py"
...
def test_complaint_orm_tenant_id_is_required():
    column = Complaint.__table__.c.tenant_id
    assert column.nullable is False
    assert column.index is True


def test_migration_enforces_only_when_all_complaints_are_attributed():
...
def test_migration_chains_from_incidents_and_never_invents_tenant():
    body = MIGRATION_PATH.read_text(encoding="utf-8")
    assert 'revision: str = "20260710_cmp_tenant_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = "20260710_inc_tenant_nn"' in body
    assert 'TABLE = "complaints"' in body
    ...
    assert (REPO_ROOT / "docs/data/complaints-tenant-backfill.md").is_file()


def test_create_path_stamps_complaint_tenant_from_caller():
    body = (REPO_ROOT / "src/domain/services/complaint_service.py").read_text(encoding="utf-8")
