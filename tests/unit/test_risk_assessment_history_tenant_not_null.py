"""Unit tests for risk_assessment_history tenant_id fail-safe backfill (WCS-TEN2)."""
...
from src.domain.models.risk_register import RiskAssessmentHistory

REPO_ROOT = Path(__file__).resolve().parents[2]
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260711_risk_assessment_history_tenant_not_null.py"
...
def test_rah_orm_tenant_id_is_required():
    column = RiskAssessmentHistory.__table__.c.tenant_id
...
def test_migration_chains_from_kri_and_never_invents_tenant():
    body = MIGRATION_PATH.read_text(encoding="utf-8")
    assert 'revision: str = "20260711_rah_tenant_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = "20260711_kri_tenant_nn"' in body
    assert 'TABLE = "risk_assessment_history"' in body
    ...
    assert (REPO_ROOT / "docs/data/risk-assessment-history-tenant-backfill.md").is_file()
...
def test_create_path_stamps_tenant_from_parent_risk():
    ...
    assert "_record_assessment" in body
    assert "tenant_id=risk.tenant_id" in body
    assert "RiskAssessmentHistory(" in body
