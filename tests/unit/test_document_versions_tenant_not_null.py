"""Unit tests for document_versions tenant_id fail-safe backfill (WCS-TEN2)."""
...
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260711_document_versions_tenant_not_null.py"
...
def test_dv_orm_tenant_id_is_required():
    column = DocumentVersion.__table__.c.tenant_id
...
def test_migration_chains_from_doc_and_never_invents_tenant():
    body = MIGRATION_PATH.read_text(encoding="utf-8")
    assert 'revision: str = "20260711_dv_tenant_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = "20260711_doc_tenant_nn"' in body
    assert 'TABLE = "document_versions"' in body
    ...
    assert (REPO_ROOT / "docs/data/document-versions-tenant-backfill.md").is_file()
