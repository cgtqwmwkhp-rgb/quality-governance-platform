...
class TestGetDashboard:
    ...
    async def test_get_dashboard_returns_all_sections(self):
        ...
        with (
            patch.object(svc, "get_standards_compliance", new_callable=AsyncMock, return_value=[
                {"standard_id": 1, "compliance_percentage": 80.0, "setup_required": False}
            ]),
            patch.object(svc, "get_isms_data", new_callable=AsyncMock, return_value={
            ...
            patch.object(svc, "get_uvdb_data", new_callable=AsyncMock, return_value={
                "total_audits": 5, "active_audits": 1, "completed_audits": 4,
                "average_score": 85.0, "latest_score": 90.0, "status": "active",
            }),
            patch.object(svc, "get_planet_mark_data", new_callable=AsyncMock, return_value={
                "status": "active", "current_year": 2025, "total_emissions": 100.0,
                "certification_status": "certified", "reduction_vs_previous": 5.0,
            }),
            patch.object(svc, "get_compliance_coverage", new_callable=AsyncMock, return_value={
                "total_clauses": 100, "covered_clauses": 80,
                "coverage_percentage": 80.0, "gaps": 20, "total_evidence_links": 200,
            }),
