...
def _service_with_lookups(
    *,
    ...
    setattr(
        service,
        "_load_asset_types",
        AsyncMock(return_value={t.name.lower(): t for t in type_list}),
    )
    loc_list = locations if locations is not None else [SimpleNamespace(id=5, name="Main Depot")]
    setattr(
        service,
        "_load_locations",
        AsyncMock(return_value={loc.name.lower(): loc for loc in loc_list}),
    )
    setattr(
        service,
        "_load_users_by_email",
        AsyncMock(
            return_value=(
                users_by_email
    ...
    setattr(
        service,
        "_load_users_by_id",
        AsyncMock(return_value=users_by_id if users_by_id is not None else {}),
    )
    setattr(
        service,
        "_existing_asset_numbers",
        AsyncMock(return_value=existing_numbers if existing_numbers is not None else set()),
    )
    return service
