"""Unit tests for policies tenant_id fail-safe backfill (WCS-TEN2)."""
...
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260711_policies_tenant_not_null.py"
...
def test_policies_orm_tenant_id_is_required():
    column = Policy.__table__.c.tenant_id
...
def test_migration_chains_from_cd_and_never_invents_tenant():
    body = MIGRATION_PATH.read_text(encoding="utf-8")
    assert 'revision: str = "20260711_pol_tenant_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = "20260711_cd_tenant_nn"' in body
    assert 'TABLE = "policies"' in body
    ...
    assert (REPO_ROOT / "docs/data/policies-tenant-backfill.md").is_file()


def test_create_path_stamps_tenant_id():
    body = (REPO_ROOT / "src/api/routes/policies.py").read_text(encoding="utf-8")
