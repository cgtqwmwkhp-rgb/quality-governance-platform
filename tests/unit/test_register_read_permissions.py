"""C-2: the RTA and document register lists require a read permission.

Both endpoints previously took ``CurrentUser`` and nothing else, so any
authenticated account in a tenant could read the whole register. ``list_rtas``
even carried a comment reading "SECURITY FIX: Always require authentication",
which is the shape of the defect: authentication was added deliberately and
authorisation was never finished. They now use the same
``require_permission(...)`` dependency their sibling write routes in the same two
modules already use, gated on ``rta:read`` and ``document:read``.

Why only two registers, when four were reported
-----------------------------------------------
``action:read`` and ``risk:read`` are in ``RESERVED_PERMISSIONS``, not
``ENFORCED_PERMISSIONS`` (``src/domain/authz/catalogue.py``), so they are **not
grantable** and are absent from the 75-token admin grant in
``docs/data/admin-role-permissions-grant.md``. Enforcing either would 403 the
admin role out of the actions and risk registers. ``rta:read`` and
``document:read`` are enforced tokens already present in that grant, so gating on
them removes access from nobody who is supposed to have it.
``test_newly_gated_tokens_are_in_the_admin_grant`` below is what holds that line.

Adding a permission gate takes access away, so the token has to exist on the
roles that legitimately read the register *before* the gate lands — not after.

Style follows ``tests/unit/test_case_view_permissions.py`` (SEC-02), which did
the same job for incidents, near-misses and complaints. The behavioural checks
here go one step further than that file: they drive the mounted router over HTTP
so the assertion is about the endpoint's real response, not only about the
``require_permission`` factory in isolation. A test that exercises the factory
alone would pass whether or not the route ever wired it up.
"""
...
GATED_REGISTERS = {
    ...
    # list_documents carries both @router.get("") and @router.get("/"), so the
    # app serves it on two paths and both had a debt declaration.
    "document:read": ("/api/v1/documents", "/api/v1/documents/"),
}

#: Registers deliberately left alone: the token is reserved and ungrantable, so
#: gating them would lock out the admin role. See the module docstring.
DEFERRED_REGISTERS = ("action:read", "risk:read")
...
def _client_for(user: _FakeUser) -> TestClient:
    """Mount both registers with authentication faked and the database emptied.
...
def test_newly_gated_tokens_are_in_the_admin_grant():
    ...
    administrators. This is why C-2 gated two registers and not four.
    """
...
def test_actions_and_risks_are_deliberately_still_ungated():
    """Record why the other two registers in the C-2 report were left alone.

    Both tokens are reserved and ungrantable, so they are absent from the admin
    grant. Promoting one into ``ENFORCED_PERMISSIONS`` adds it to
    ``ADMIN_ROLE_PERMISSIONS`` automatically, which makes
    ``docs/data/admin-role-permissions-grant.md`` stale and needs a fresh
    ``UPDATE roles`` applied by a human before the gate can land safely. When
    that happens, gate the endpoint and delete this test.
    """
    for token in DEFERRED_REGISTERS:
        assert token in RESERVED_PERMISSIONS, (
            f"{token} is no longer reserved. If it has been promoted to an enforced token and "
            "the admin grant re-applied, gate the matching register list and remove this test."
        )
        assert token not in ADMIN_ROLE_PERMISSIONS

    assert "action:read" not in _permission_depends(REPO / "src/api/routes/actions.py")
    assert "risk:read" not in _permission_depends(REPO / "src/api/routes/risks.py")
