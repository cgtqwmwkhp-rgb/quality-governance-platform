"""Unit tests for documents tenant_id fail-safe backfill (WCS-TEN2)."""
...
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260711_documents_tenant_not_null.py"
...
def test_documents_orm_tenant_id_is_required():
    column = Document.__table__.c.tenant_id
...
def test_migration_chains_from_pol_and_never_invents_tenant():
    body = MIGRATION_PATH.read_text(encoding="utf-8")
    assert 'revision: str = "20260711_doc_tenant_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = "20260711_pol_tenant_nn"' in body
    assert 'TABLE = "documents"' in body
    assert 'PARENT = "users"' in body
    assert 'PARENT_KEY = "created_by_id"' in body
    assert "reviewed_by_id" in body
    assert "FAIL-SAFE" in body
    assert "tenant_id = 1" not in body
    assert (REPO_ROOT / "docs/data/documents-tenant-backfill.md").is_file()


def test_create_path_stamps_tenant_id():
    body = (REPO_ROOT / "src/api/routes/documents.py").read_text(encoding="utf-8")
