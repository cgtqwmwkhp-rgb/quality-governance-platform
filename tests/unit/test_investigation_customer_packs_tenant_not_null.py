"""Unit tests for investigation_customer_packs tenant_id fail-safe backfill (WCS-TEN2)."""
...
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260710_investigation_customer_packs_tenant_not_null.py"
...
def test_investigation_customer_pack_orm_tenant_id_is_required():
    column = InvestigationCustomerPack.__table__.c.tenant_id
    assert column.nullable is False
    assert column.index is True


def test_migration_enforces_only_when_all_packs_are_attributed():
...
def test_migration_chains_from_investigation_runs_and_never_invents_tenant():
    body = MIGRATION_PATH.read_text(encoding="utf-8")
    assert 'revision: str = "20260710_inv_pack_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = "20260710_ir_tenant_nn"' in body
    assert 'TABLE = "investigation_customer_packs"' in body
    ...
    assert (REPO_ROOT / "docs/data/investigation-customer-packs-tenant-backfill.md").is_file()
