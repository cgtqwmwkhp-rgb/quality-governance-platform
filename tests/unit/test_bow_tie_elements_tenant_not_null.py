"""Unit tests for bow_tie_elements tenant_id fail-safe backfill (WCS-TEN2)."""
...
from src.domain.models.risk_register import BowTieElement

REPO_ROOT = Path(__file__).resolve().parents[2]
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260711_bow_tie_elements_tenant_not_null.py"
...
def test_bow_tie_orm_tenant_id_is_required():
    column = BowTieElement.__table__.c.tenant_id
...
def test_migration_chains_from_risks_v2_and_never_invents_tenant():
    body = MIGRATION_PATH.read_text(encoding="utf-8")
    assert 'revision: str = "20260711_bow_tenant_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = "20260711_rv2_tenant_nn"' in body
    assert 'TABLE = "bow_tie_elements"' in body
    ...
    assert (REPO_ROOT / "docs/data/bow-tie-elements-tenant-backfill.md").is_file()
...
def test_create_path_stamps_tenant_from_parent_risk():
    ...
    assert "add_bow_tie_element" in body
