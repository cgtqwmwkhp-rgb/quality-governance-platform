"""Unit tests for obsolete_document_records tenant_id fail-safe backfill (WCS-TEN2)."""
...
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260711_obsolete_document_records_tenant_not_null.py"
...
def test_odr_orm_tenant_id_is_required():
    column = ObsoleteDocumentRecord.__table__.c.tenant_id
...
def test_migration_chains_from_dv_and_never_invents_tenant():
    body = MIGRATION_PATH.read_text(encoding="utf-8")
    assert 'revision: str = "20260711_odr_tenant_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = "20260711_dv_tenant_nn"' in body
    assert 'TABLE = "obsolete_document_records"' in body
