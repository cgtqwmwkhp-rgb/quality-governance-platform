"""Unit tests for risks tenant_id fail-safe backfill (WCS-TEN2)."""
...
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260711_risks_tenant_not_null.py"
...
def test_risk_orm_tenant_id_is_required():
    column = Risk.__table__.c.tenant_id
    assert column.nullable is False
    assert column.index is True


def test_migration_enforces_only_when_all_risks_are_attributed():
...
def test_migration_chains_from_complaints_and_never_invents_tenant():
    body = MIGRATION_PATH.read_text(encoding="utf-8")
    assert 'revision: str = "20260711_risks_tenant_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = "20260710_cmp_tenant_nn"' in body
    assert 'TABLE = "risks"' in body
    ...
    assert (REPO_ROOT / "docs/data/risks-tenant-backfill.md").is_file()


def test_create_path_stamps_risk_tenant_from_caller():
