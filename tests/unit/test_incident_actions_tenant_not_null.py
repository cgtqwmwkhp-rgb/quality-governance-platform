"""Unit tests for incident_actions tenant_id fail-safe backfill (WCS-TEN2)."""
...
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260710_incident_actions_tenant_not_null.py"
...
def test_incident_action_orm_tenant_id_is_required():
    column = IncidentAction.__table__.c.tenant_id
...
def test_migration_chains_from_audit_findings_and_never_invents_tenant():
    body = MIGRATION_PATH.read_text(encoding="utf-8")
    assert 'revision: str = "20260710_ia_tenant_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = ("20260710_af_tenant_nn", "20260710_audit_meta")' in body
    assert 'TABLE = "incident_actions"' in body
    assert 'PARENT = "incidents"' in body
    assert 'PARENT_KEY = "incident_id"' in body
    assert "FAIL-SAFE" in body
    assert "tenant_id = 1" not in body
    assert "SET tenant_id = 1" not in body.upper()
    assert (REPO_ROOT / "docs/data/incident-actions-tenant-backfill.md").is_file()
