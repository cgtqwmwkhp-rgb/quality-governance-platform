"""Path-to-10 Preferred S10: Mistral analysis via catalog ``mistral_analysis`` breaker."""
...
def _service(*, api_key: str = "test-key") -> MistralAnalysisService:
    svc = MistralAnalysisService()
    svc.api_key = api_key
...
@pytest.mark.asyncio
async def test_analyze_text_registers_preferred_mistral_breaker_on_success() -> None:
    svc = _service()
    mock_resp = MagicMock()
    ...
    mock_resp.json.return_value = {
        "choices": [
            {
                "message": {
                    "content": '{"scheme":"iso","findings":[],"score_breakdown":[]}',
                }
            }
        ]
    }
    ...
    with patch("httpx.AsyncClient", return_value=mock_client):
        result = await svc.analyze_text(_long_text(), assurance_scheme="iso")
...
@pytest.mark.asyncio
async def test_repeated_analyze_failures_open_mistral_circuit() -> None:
    svc = _service()

    mock_client = AsyncMock()
    mock_client.post = AsyncMock(side_effect=RuntimeError("mistral unavailable"))
    mock_client.__aenter__ = AsyncMock(return_value=mock_client)
    mock_client.__aexit__ = AsyncMock(return_value=False)

    with patch("httpx.AsyncClient", return_value=mock_client):
        for _ in range(5):
            result = await svc.analyze_text(_long_text())
            assert result.provider_status == "failed"

    assert get_upstream_breaker("mistral_analysis").state == CircuitState.OPEN

    with patch("httpx.AsyncClient", return_value=mock_client):
        open_result = await svc.analyze_text(_long_text())
...
@pytest.mark.asyncio
async def test_analyze_text_does_not_invent_api_key_when_unconfigured() -> None:
