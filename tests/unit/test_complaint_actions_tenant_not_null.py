"""Unit tests for complaint_actions tenant_id fail-safe backfill (WCS-TEN2)."""
...
MIGRATION_PATH = REPO_ROOT / "alembic/versions/20260710_complaint_actions_tenant_not_null.py"
...
def test_complaint_action_orm_tenant_id_is_required():
    column = ComplaintAction.__table__.c.tenant_id
...
def test_migration_chains_from_incident_actions_and_never_invents_tenant():
    body = MIGRATION_PATH.read_text(encoding="utf-8")
    assert 'revision: str = "20260710_ca_tenant_nn"' in body
    assert 'down_revision: Union[str, Sequence[str], None] = "20260710_ia_tenant_nn"' in body
    assert 'TABLE = "complaint_actions"' in body
    assert 'PARENT = "complaints"' in body
    assert 'PARENT_KEY = "complaint_id"' in body
    assert "FAIL-SAFE" in body
    assert "tenant_id = 1" not in body
    assert "SET tenant_id = 1" not in body.upper()
    assert (REPO_ROOT / "docs/data/complaint-actions-tenant-backfill.md").is_file()
