...
class TestLoginErrorCodes:
    ...
    def test_401_response_has_proper_structure(self, client):
        ...
        assert (
            has_error_field or has_nested_error
    ...
    def test_missing_fields_returns_422(self, client):
        ...
        has_nested = isinstance(data.get("error"), dict) and (
            "message" in data["error"] or "details" in data["error"]
        )
        assert (
            has_top_level or has_nested
        ), f"Response missing validation details: {data}"
