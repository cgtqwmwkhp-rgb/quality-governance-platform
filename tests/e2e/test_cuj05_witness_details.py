...
class TestCUJ05WitnessDetails:
    ...
    def test_add_witness_to_rta(self, client, auth_headers):
        ...
        assert resp.status_code in (200, 201, 401, 403, 404, 422, 500, 503), f"RTA creation: {resp.text}"
        ...
        assert patch.status_code in (200, 401, 403, 404, 422, 500, 503), patch.text
    ...
    def test_running_sheet_entry_for_witness(self, client, auth_headers):
        ...
        assert entry.status_code in (200, 201, 401, 403, 404, 422, 500, 503), entry.text
