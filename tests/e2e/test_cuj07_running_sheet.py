...
class TestCUJ07RunningSheet:
    ...
    def test_add_running_sheet_entry(self, client, auth_headers):
        ...
        if rta.status_code not in (200, 201):
            assert rta.status_code in (401, 403, 404, 422, 500, 503)
            return
        ...
        assert resp.status_code in (200, 201, 401, 403, 404, 422, 500, 503), resp.text
    ...
    def test_running_sheet_timeline_ordered(self, client, auth_headers):
        ...
        if sheet.status_code != 200:
            assert sheet.status_code in (401, 403, 404, 500, 503)
            return
