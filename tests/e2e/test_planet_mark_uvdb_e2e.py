...
PHASE 3 FIX (PR #104):
- GOVPLAT-004 RESOLVED: Now uses the blessed async_client fixture
...
class TestPlanetMarkDashboardFlow:
    ...
    @pytest.mark.asyncio
    async def test_dashboard_loads_and_shows_relevant_data(self, async_client):
    ...
    @pytest.mark.asyncio
    async def test_years_navigation_flow(self, async_client):
        ...
        if years_data["years"]:
            ...
            detail_response = await async_client.get(f"/api/v1/planet-mark/years/{year_id}")
    ...
    @pytest.mark.asyncio
    async def test_scope3_exploration_flow(self, async_client):
        """User can explore Scope 3 categories for any year."""
        # Get years
        years_response = await async_client.get("/api/v1/planet-mark/years")
    ...
    @pytest.mark.asyncio
    async def test_error_handling_for_invalid_year(self, async_client):
...
class TestPlanetMarkDataQualityFlow:
    ...
    @pytest.mark.asyncio
    async def test_data_quality_assessment_structure(self, async_client):
...
class TestUVDBProtocolExplorationFlow:
    ...
    @pytest.mark.asyncio
    async def test_protocol_overview_to_section_details(self, async_client):
        """User can view protocol overview and drill into sections."""
        # Step 1: Get protocol overview
        protocol_response = await async_client.get("/api/v1/uvdb/protocol")
        ...
        sections_response = await async_client.get("/api/v1/uvdb/sections")
    ...
    @pytest.mark.asyncio
    async def test_iso_mapping_exploration(self, async_client):
...
class TestUVDBAuditManagementFlow:
    ...
    @pytest.mark.asyncio
    async def test_audit_listing_and_filtering(self, async_client):
        """User can list and filter audits."""
        # List all audits
        all_response = await async_client.get("/api/v1/uvdb/audits")
        assert all_response.status_code == 200
        all_data = all_response.json()
        assert "audits" in all_data

        # Filter by status (if any audits exist)
        filtered_response = await async_client.get("/api/v1/uvdb/audits?status=completed")
        assert filtered_response.status_code == 200

    @pytest.mark.asyncio
    async def test_dashboard_summary_matches_list(self, async_client):
        """Dashboard summary counts should be consistent with list endpoint."""
        # Get dashboard
        dashboard_response = await async_client.get("/api/v1/uvdb/dashboard")
        assert dashboard_response.status_code == 200
        dashboard_data = dashboard_response.json()

        # Get list
        list_response = await async_client.get("/api/v1/uvdb/audits")
...
class TestDeterministicRendering:
    ...
    @pytest.mark.asyncio
    async def test_sections_list_is_deterministic(self, async_client):
    ...
    async def test_audits_list_is_deterministic(self, async_client):
        ...
        responses = []
        for _ in range(3):
            response = await async_client.get("/api/v1/uvdb/audits?limit=10")
            assert response.status_code == 200
            responses.append(response.json())
...
class TestErrorStateBoundedResponses:
    ...
    @pytest.mark.asyncio
    async def test_validation_error_is_helpful(self, async_client):
        ...
        if response.status_code == 422:
            data = response.json()
            # Accept both FastAPI default (detail) and custom envelope (message/details)
