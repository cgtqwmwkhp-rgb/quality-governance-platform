...
class TestPortalRoutingCorrectness:
    ...
    @pytest.mark.asyncio
    async def test_incident_portal_routes_to_incidents_table(
    ...
    async def test_rta_portal_routes_to_rta_table(
        ...
        assert data["reference_number"].startswith("RTA-"), \
            f"Expected RTA- prefix, got: {data['reference_number']}"
        ...
        # Verify source_form_id is set for audit
        rta = (await db_session.execute(
            select(RoadTrafficCollision).where(
                RoadTrafficCollision.reference_number == data["reference_number"]
            )
        )).scalar_one_or_none()
        assert rta is not None, "RTA record not found"
        assert rta.source_form_id == "portal_rta_v1", \
    ...
    async def test_near_miss_portal_routes_to_near_miss_table(
        ...
        # Get initial count
        initial_count = (await db_session.execute(
            select(func.count()).select_from(NearMiss)
        )).scalar() or 0
        ...
        assert data["reference_number"].startswith("NM-"), \
            f"Expected NM- prefix, got: {data['reference_number']}"
        ...
        # Verify source_form_id is set for audit
        near_miss = (await db_session.execute(
            select(NearMiss).where(NearMiss.reference_number == data["reference_number"])
        )).scalar_one_or_none()
        assert near_miss is not None, "Near miss record not found"
        assert near_miss.source_form_id == "portal_near_miss_v1", \
    ...
    async def test_complaint_portal_routes_to_complaint_table(
        ...
        assert data["reference_number"].startswith("COMP-"), \
            f"Expected COMP- prefix, got: {data['reference_number']}"
        ...
        complaint = (await db_session.execute(
            select(Complaint).where(Complaint.reference_number == data["reference_number"])
        )).scalar_one_or_none()
        assert complaint is not None, "Complaint record not found"
        assert complaint.source_form_id == "portal_complaint_v1", \
    ...
    async def test_rta_dashboard_does_not_show_incidents(
        ...
        assert incident_ref not in reference_numbers, \
    ...
    async def test_incidents_dashboard_does_not_show_rtas(
        ...
        assert rta_ref not in reference_numbers, \
    ...
    async def test_unknown_report_type_rejected(self, async_client: AsyncClient):
        ...
        assert "invalid report_type" in error_detail.lower() or "must be" in error_detail.lower(), \
    ...
    @pytest.mark.asyncio
    async def test_portal_routing_is_deterministic(
        self, async_client: AsyncClient, db_session: AsyncSession
    ):
    ...
    async def test_all_portal_types_have_complete_mapping(self, async_client: AsyncClient):
        ...
        for portal_type, contract in self.PORTAL_ROUTING_CONTRACT.items():
            ...
            response = await async_client.post("/api/v1/portal/reports/", json=payload)
            assert response.status_code == 201, \
                f"Portal type '{portal_type}' submission failed: {response.json()}"
...
class TestSourceFormAuditTrail:
    ...
    @pytest.mark.asyncio
    async def test_source_form_id_enables_audit_query(
        self, async_client: AsyncClient, db_session: AsyncSession
    ):
        ...
        incident_count = (await db_session.execute(
        ...
        rta_count = (await db_session.execute(
            select(func.count()).select_from(RoadTrafficCollision).where(
                RoadTrafficCollision.source_form_id == "portal_rta_v1"
            )
        )).scalar() or 0
