...
class TestFromRecordEndpoint:
    ...
    async def test_from_record_rejects_empty_body(self, client: AsyncClient):
    ...
    async def test_from_record_validates_source_type(self, client: AsyncClient):
    ...
    async def test_from_record_validates_source_id_positive(self, client: AsyncClient):
    ...
    async def test_from_record_validates_title_not_empty(self, client: AsyncClient):
...
class TestSourceRecordsEndpoint:
    ...
    async def test_source_records_requires_source_type(self, client: AsyncClient):
        """Test source-records requires source_type query param."""
        response = await client.get("/api/v1/investigations/source-records")
        # Should get 422 for missing required query param
        assert response.status_code == 422

    async def test_source_records_validates_source_type(self, client: AsyncClient):
        """Test source-records validates source_type."""
        response = await client.get(
            "/api/v1/investigations/source-records",
            params={"source_type": "invalid_type"},
        )
        # Should get 401 (auth first) or 400 (validation) after auth
