...
async def get_audit_responses(
    ...
    stmt = select(UVDBAuditResponse).where(UVDBAuditResponse.audit_id == audit_id)
    result = await db.execute(stmt)
...
async def get_audit_kpis(
    ...
    stmt = select(UVDBKPIRecord).where(UVDBKPIRecord.audit_id == audit_id).order_by(UVDBKPIRecord.year.desc())
    result = await db.execute(stmt)
...
async def get_uvdb_dashboard(
    ...
    result = await db.execute(completed_with_score_stmt)
