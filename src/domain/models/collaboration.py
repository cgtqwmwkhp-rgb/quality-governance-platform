...
class CollaborativeDocument(Base):
    ...
    field_name: Mapped[str] = mapped_column(
        String(100), default="content"
    )  # Which field this document represents

    # Yjs state
    yjs_state: Mapped[bytes] = mapped_column(
        LargeBinary, nullable=True
    )  # Encoded Yjs document state
    yjs_state_vector: Mapped[bytes] = mapped_column(
        LargeBinary, nullable=True
    ...
    sessions = relationship(
        "CollaborativeSession", back_populates="document", cascade="all, delete-orphan"
    )
...
class CollaborativeSession(Base):
    ...
    document_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("collaborative_documents.id"), nullable=False
    )
    ...
    )  # Unique color for cursor

    # Cursor position
    cursor_position: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    )  # {start, end, path}
    selection_range: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)

    # Current view
    current_field: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True
    )  # Which field user is editing
...
class CollaborativeChange(Base):
    ...
    )  # insert, delete, format, etc.
    change_data: Mapped[dict] = mapped_column(
        JSON, nullable=False
    )  # Yjs update or structured change

    # Position
    path: Mapped[Optional[str]] = mapped_column(
        String(500), nullable=True
    )  # JSON path to change location
...
class Comment(Base):
    ...
    parent_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("document_comments.id"), nullable=True
    )
    thread_id: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )  # Root comment ID for the thread
    ...
    anchor_path: Mapped[Optional[str]] = mapped_column(
        String(500), nullable=True
    )  # JSON path
...
class Presence(Base):
    ...
    current_entity_type: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True
    )
