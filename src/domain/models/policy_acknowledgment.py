...
class PolicyAcknowledgmentRequirement(Base, TimestampMixin):
    ...
    required_departments: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)  # List of departments
    required_roles: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)  # List of roles
    required_user_ids: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)  # Specific user IDs
    
    # Timing
    due_within_days: Mapped[int] = mapped_column(Integer, default=30)  # Days after assignment
    reminder_days_before: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)  # [7, 3, 1] = remind at 7, 3, 1 days before due
    
    # Re-acknowledgment
    re_acknowledge_on_update: Mapped[bool] = mapped_column(Boolean, default=True)
    re_acknowledge_period_months: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # Periodic re-read
    
    # Quiz settings (if acknowledgment_type is QUIZ)
    quiz_questions: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)
    quiz_passing_score: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # Percentage
...
class PolicyAcknowledgment(Base, TimestampMixin):
    ...
    time_spent_seconds: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # Track reading time
    ...
    acceptance_statement: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # What they agreed to
    signature_data: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # Digital signature (base64 or reference)
    ip_address: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)  # For audit trail
    user_agent: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)  # Browser info
...
class DocumentReadLog(Base, TimestampMixin):
    ...
    # What was read
    document_type: Mapped[str] = mapped_column(String(50), nullable=False, index=True)  # policy, procedure, standard
    ...
    scroll_percentage: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # 0-100
