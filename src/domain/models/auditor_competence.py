...
class AuditorProfile(Base, TimestampMixin, AuditTrailMixin):
    ...
    competence_level: Mapped[CompetenceLevel] = mapped_column(
        SQLEnum(CompetenceLevel, native_enum=False), 
        default=CompetenceLevel.TRAINEE
    )
    ...
    # Languages
    languages: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)  # [{"language": "English", "level": "native"}]
...
class AuditorCertification(Base, TimestampMixin):
    ...
    # Level
    certification_level: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)  # Internal Auditor, Lead Auditor
...
class AuditAssignmentCriteria(Base, TimestampMixin):
    ...
    audit_type: Mapped[str] = mapped_column(String(100), nullable=False, index=True)  # ISO 9001 audit, Safety audit, etc.
    ...
    minimum_auditor_level: Mapped[CompetenceLevel] = mapped_column(
        SQLEnum(CompetenceLevel, native_enum=False), 
        default=CompetenceLevel.AUDITOR
    )
