...
class KeyRiskIndicator(Base, TimestampMixin, AuditTrailMixin):
    ...
    measurements: Mapped[List["KRIMeasurement"]] = relationship(
        "KRIMeasurement", back_populates="kri", cascade="all, delete-orphan",
...
class KRIMeasurement(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    kri_id: Mapped[int] = mapped_column(ForeignKey("key_risk_indicators.id", ondelete="CASCADE"), nullable=False, index=True)
