...
class SignatureRequest(Base):
    ...
    )  # draft, pending, in_progress, completed, declined, expired
    ...
    # Reminders
    reminder_frequency: Mapped[Optional[int]] = mapped_column(
    ...
    signed_document_hash: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True
    )
    ...
    signers = relationship(
        "SignatureRequestSigner", back_populates="request", cascade="all, delete-orphan"
    )
...
class SignatureRequestSigner(Base):
    ...
    request_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("signature_requests.id"), nullable=False
    )
    ...
    signer_role: Mapped[str] = mapped_column(
        String(50), default="signer"
    )  # signer, approver, witness, cc
    ...
    signature_type: Mapped[Optional[str]] = mapped_column(
        String(20), nullable=True
    )  # drawn, typed, uploaded
    signature_data: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )  # Base64 image or typed name
...
class Signature(Base):
    ...
    request_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("signature_requests.id"), nullable=True
    )
    signer_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("signature_request_signers.id"), nullable=True
    )
    ...
    )  # SHA-256 of document at signing

    # Signer
    signer_user_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("users.id"), nullable=True
    )
    ...
    signature_image: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )  # Base64
    signature_text: Mapped[Optional[str]] = mapped_column(
    ...
    # Legal validity
    legal_statement: Mapped[str] = mapped_column(
        Text, nullable=False
    ...
    signature_hash: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # Hash of signature data + document
...
class SignatureTemplate(Base):
    ...
    # Template document
    document_template: Mapped[Optional[bytes]] = mapped_column(
        LargeBinary, nullable=True
    )
    document_filename: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
...
class SignatureAuditLog(Base):
    ...
    __table_args__ = (Index("ix_sig_audit_request", "request_id", "created_at"),)
