...
class CarbonReportingYear(Base):
    ...
    baseline_year_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("carbon_reporting_year.id"), nullable=True)
...
class EmissionSource(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    reporting_year_id: Mapped[int] = mapped_column(Integer, ForeignKey("carbon_reporting_year.id", ondelete="CASCADE"), nullable=False)
