...
class ExecutiveDashboardService:
    ...
    async def get_full_dashboard(
        ...
        near_miss_summary = await self._safe_call(
            self._get_near_miss_summary(cutoff), empty_summary
        )
        complaint_summary = await self._safe_call(
            self._get_complaint_summary(cutoff), empty_summary
        )
        rta_summary = await self._safe_call(
            self._get_rta_summary(cutoff), empty_summary
        )
        ...
        compliance_summary = await self._safe_call(
            self._get_compliance_summary(), empty_compliance
        )
