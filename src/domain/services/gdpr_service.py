...
class GDPRService:
        ...
        try:
            ...
            if model_name == "IncidentAction":
                ...
                stmt = (
                    ...
                    .where(user_col == user_id, Incident.tenant_id == tenant_id)
                    .limit(1000)
                )
                ...
                stmt = (
                    ...
                    .where(user_col == user_id, Complaint.tenant_id == tenant_id)
                    .limit(1000)
                )
    ...
    async def restrict_processing(self, user_id: int, record_type: str, record_id: int) -> dict[str, Any]:
        ...
        This is a compliance stub that logs the restriction request and
        returns confirmation.  A full implementation will set a
        ``processing_restricted`` flag on the target record.
        """
        timestamp = datetime.now(timezone.utc).isoformat()
        logger.info(
            "Art. 18 restriction requested: user_id=%d record_type=%s record_id=%d",
            user_id,
            record_type,
            record_id,
        )
        return {
            "status": "restriction_recorded",
            "user_id": user_id,
