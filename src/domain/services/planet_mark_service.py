...
class PlanetMarkService:
    ...
    def get_emission_factor(activity_type: str) -> dict:
        ...
        return EMISSION_FACTORS.get(
            activity_type, {"factor": 0, "unit": "", "source": ""}
        )

    @staticmethod
    def calculate_co2e(
        activity_value: float, activity_type: str
    ...
    def calculate_overall_data_quality(
        scope1_score: float, scope2_score: float, scope3_score: float
    ) -> int:
    ...
    def calculate_fleet_co2e(fuel_litres: float, fuel_type: str) -> tuple[float, dict]:
        ...
        ef = EMISSION_FACTORS.get(
            f"{fuel_type.lower()}_litres", EMISSION_FACTORS["diesel_litres"]
        )
        return fuel_litres * float(ef["factor"]), ef

    @staticmethod
    def calculate_fuel_efficiency(
        fuel_litres: float, mileage: float | None
    ) -> float | None:
    ...
    def calculate_certification_readiness(required_evidence: list[dict]) -> float:
        ...
        required_complete = sum(
            1 for r in required_evidence if r.get("required") and r.get("uploaded")
        )
    ...
    async def get_reporting_year_detail(
        ...
        return {
            ...
            "certification": {
                ...
                "certification_date": (
                    year.certification_date.isoformat()
                    if year.certification_date
                    else None
                ),
                "expiry_date": (
                    year.expiry_date.isoformat() if year.expiry_date else None
                ),
            },
        }
            ...
            "sources": [
                {
                    ...
                    "percentage": (
                        round((float(s.co2e_tonnes) / total * 100), 1)
                        if total > 0
                        else 0
                    ),
            ...
            "categories": [
                {
                    ...
                    "percentage": (
                        round((float(c.total_co2e) / total * 100), 1)
                        if total > 0
                        else 0
                    ),
        ...
        *,
        ...
        overdue = len(
            [
                a
                for a in actions
                if a.status != "completed" and a.time_bound and a.time_bound < now
            ]
        )
            ...
            "actions": [
                {
                    ...
                    and bool(a.time_bound and a.time_bound < now),
                }
                for a in actions
            ],
        }
    ...
    async def create_improvement_action(
        ...
        count_result = await db.execute(
            ...
            .where(ImprovementAction.reporting_year_id == year_id)
        )
    ...
    async def get_data_quality_assessment(
        ...
        scope1 = PlanetMarkService.calculate_scope_quality(
            [s for s in sources if s.scope == "scope_1"]
        )
        scope2 = PlanetMarkService.calculate_scope_quality(
            [s for s in sources if s.scope == "scope_2"]
        )
        scope3 = PlanetMarkService.calculate_scope_quality(
            [s for s in sources if s.scope == "scope_3"]
        )

        overall = PlanetMarkService.calculate_overall_data_quality(
            scope1["score"], scope2["score"], scope3["score"]
        )
    ...
    async def get_fleet_summary(
        ...
        for v in vehicles.values():
            efficiency = PlanetMarkService.calculate_fuel_efficiency(
                v["total_litres"], v["total_mileage"]
            )
            v["litres_per_100km"] = (
    ...
    async def get_certification_status(
        ...
        readiness = PlanetMarkService.calculate_certification_readiness(
            required_evidence
        )
    ...
    async def get_carbon_dashboard(db: AsyncSession) -> dict[str, Any] | None:
        ...
        if len(years) >= 2:
            ...
            if prev_year.emissions_per_fte and current_year.emissions_per_fte:
                yoy_change = (
                    (
                        float(current_year.emissions_per_fte)
        ...
        overdue_actions = [
            a
            for a in actions
            if a.status != "completed" and a.time_bound and a.time_bound < dashboard_now
        ]
        ...
        return {
            ...
            "data_quality": {
                "scope_1_2": int(current_year.scope_1_data_quality or 0)
                + int(current_year.scope_2_data_quality or 0),
    ...
    @staticmethod
    async def recalculate_year_totals(
        db: AsyncSession, year: CarbonReportingYear
    ) -> None:
