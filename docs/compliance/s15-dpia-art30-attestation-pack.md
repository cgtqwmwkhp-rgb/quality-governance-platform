...
| International transfers | Hosting is UK/EEA across **two** regions — West Europe (application processing, uploaded documents at rest, secrets, telemetry) and UK South (database, cache) — verified 2026-07-28 and published as `international_transfers.hosting_regions_by_purpose`; AI regions and mechanisms marked `unknown_not_established_in_repository` | Establish each AI vendor's region and safeguard — production AI keys are already live, so this is remediation, not a precondition. The hosting split is an accuracy defect, not an unlawful transfer: **no SCC/IDTA is engaged by it** |
