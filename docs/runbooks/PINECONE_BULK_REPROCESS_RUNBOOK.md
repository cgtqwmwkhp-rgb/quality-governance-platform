...
| `Stale vector cleanup incomplete for document N` in `error_log` | Pinecone delete-by-ID failed after a successful upsert | Indexing succeeded; the listed vectors are orphans. Re-run reprocess for that document to retry cleanup — deletes are idempotent |
