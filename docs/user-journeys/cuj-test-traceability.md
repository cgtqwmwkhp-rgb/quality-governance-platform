...
| **CUJ-02** | Manager creates CAPA action from incident | `tests/e2e/test_cuj02_capa_from_incident.py` (`TestCUJ02CAPAFromIncident`); `tests/e2e/test_enterprise_e2e.py` (`TestAuditLifecycleE2E` — lifecycle narrative includes CAPA; primarily audit/findings endpoints); `tests/e2e/test_full_workflow.py` (`TestAuditWorkflow`); `tests/uat/test_stage1_basic_workflows.py` (`TestIncidentManagementWorkflows` — auth/list/update guards, not full CAPA create) | E2E: ~4–6 (dedicated CUJ + aligned); UAT: indirect | **Partial** — Dedicated E2E test creates CAPA from incident with source_id linkage assertion; full round-trip pending API contract stabilization. |
| **CUJ-03** | Driver completes daily vehicle checklist | _No dedicated file found under `tests/e2e/` or `tests/uat/` in current tree_; `tests/smoke/test_enterprise_smoke.py` may touch fleet health indirectly | 0 journey-specific; golden fixtures added | **Partial** — Golden fixture data for driver and vehicle added (`tests/fixtures/golden/`); dedicated E2E/UAT test pending fleet/daily-checklist endpoint availability. |
| **CUJ-04** | User reports an RTA with third party details | `tests/e2e/test_portal_e2e.py` (`TestRTAReporting::test_submit_rta_report`); `tests/e2e/test_full_workflow.py` (`TestEmployeePortalFlow::test_rta_report_flow`); `tests/uat/test_stage1_basic_workflows.py` (`TestRTAManagementWorkflows` — authenticated RTA API auth guards) | E2E: 2; UAT: ~5 (auth/contract, not full portal RTA happy path) | **Partial** — *RTA portal contract stabilization:* tests still tolerate 404s where `/api/v1/portal/rta` is not final; full happy-path coverage will follow contract freeze and removal of 404-tolerant assertions (see follow-up #2). |
| **CUJ-05** | Witness details added to RTA | `tests/uat/test_stage1_basic_workflows.py` (`TestRTAManagementWorkflows` — generic RTA endpoints) | UAT: indirect (~2); unit: factory smoke | **Partial** — Factory validation covers RTA model build via `test_factory_build_validation.py`; explicit witness sub-resource tests pending RTA portal/API contract stabilization. |
| **CUJ-06** | User uploads evidence photos | `tests/e2e/test_cuj06_evidence_upload.py` (`TestCUJ06EvidenceUpload`); `tests/e2e/test_enterprise_e2e.py` / `test_full_workflow.py` (evidence endpoints) | E2E: 2 (dedicated upload + list verification); indirect: ~2 | **Partial** — Dedicated E2E test uploads evidence to incident and verifies asset appears in list via `test_cuj06_evidence_upload.py`; environment-dependent upload path may limit CI coverage. |
| **CUJ-07** | Running sheet entry added to RTA | `tests/uat/test_stage1_basic_workflows.py` (`TestRTAManagementWorkflows::test_uat_035_rta_actions_require_auth` — actions endpoint auth only) | UAT: 1; unit: factory smoke | **Partial** — Factory validation covers RTAAction model build; running-sheet mutation tests pending actions contract and portal behaviour finalization (pairs with CUJ-04). |
