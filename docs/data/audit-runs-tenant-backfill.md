# Audit runs tenant backfill (WCS-TEN2 / C-01 Phase 2)

## Scope

Single table family only: `audit_runs.tenant_id`, inherited from
`audit_templates` via the NOT NULL FK `audit_runs.template_id`.

Do **not** invent `tenant_id=1`. Parent `audit_templates.tenant_id` may still be
...
## Migration `20260710_ar_tenant_nn`

1. Backfill NULL runs from the parent template when `audit_templates.tenant_id IS NOT NULL`.
...
`AuditTemplate.tenant_id` stays nullable. Risks / incidents / complaints cores
...
JOIN audit_templates AS t ON t.id = r.template_id
...
Downgrade restores `audit_runs.tenant_id` to nullable when it was NOT
